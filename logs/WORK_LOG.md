## Session 339 — 2026-09-02 (the verification origin with no publisher · the status baked into a PNG)

**Intent:** Run the full arc — `/start → /audit → /implement → /closeout` — under founder authorization for a direct push to `main` and a full production deploy.

- Ran the S339 start/canon/frontier preflight. Canon synced against the live `STUDIO_CANON.md` (54 canons, 4 pending review), frontier radar current at 10/10 official sources, secrets 51/75. Three studio-ops-only scripts are not propagated here (`start-sync`, `run-maintenance`, `arc-profile`) and were noted rather than skipped silently.
- **Root-caused the top-ranked P1 by disproving its premise.** S338 asked what deploys Hetzner staging and why it stopped. Nothing ever did: the origin is named 14 times across the workflows, every reference a read, and the only publisher was invoked by zero workflows and reachable only through an npm alias nothing called. CANON-007 had been running backwards — the release ceremony verifying against a tree five days older than the one it was clearing.
- **Refreshed staging with the credential that had been READY the whole time** (CANON-019 try-first): 340 overlays, 25 safe removals, exact-byte verified, identity untouched. Advertised surface 115 → 135/135, zero missing. Flipped `surfaceParity` from reported to gating and wired it into `classifyStatus()`, with the remedy command published on the artifact (31/31, was 26).
- **Gated the class:** `check-verification-origin-publisher.mjs` — every workflow-named origin needs a declared publisher that exists, references the origin, and is reachable by the exact route it claims. 19/19, and proven live: with the staging declaration removed it reproduces the real defect and names all five workflows that verify against that origin.
- **Declined to make it "automated" by widening blast radius.** `gh secret set HETZNER_SSH_KEY` would have put a root key on the staging box reachable from every workflow run. Declared `operator` with the reason on the record instead; the gate refuses an operator claim whose npm script does not actually exist and actually run the publisher, so the declaration cannot decay into a fiction.
- **Turned the S338 receipt round-trip property into a harness** (`scripts/lib/receipt-roundtrip.mjs`) paired with its own proof-of-liveness, and gated its use (`check-receipt-roundtrip-coverage.mjs`, 15/15). Audited every re-derive site in `scripts/`: exactly one exists, so the class is closed everywhere it occurs; the gate is for the second site, which is where all three historical field losses happened. `build-deploy-currency` refactored onto it and holds 87/87.
- **Found the home page advertising three shipped products as unfinished.** Velaxis and Vorn were filed under Forge and PromoGrind wore a Forge badge under a Sparked heading, while the catalog, the nav and all three destination pages said SPARKED. Moved both, removed all 11 in-tier badges, gated placement and badge-absence against the canonical feeds (`check-home-portfolio-status-coherence.mjs`, 20/20).
- **CANON-053 then found the defect the source could not show.** After all of the above the rendered capture still showed a doubled status label on the hero tiles. A Playwright DOM probe found exactly one status node per tile — ruling out markup and client-side injection together — which pointed at the asset: `build-game-covers.mjs` rasterized the status word into every cover, directly under the absolutely-positioned badge, clipped to a bare `S` on narrow tiles by `background-size: cover`. The word came from a hardcoded array duplicating `data/game-registry.json`, so it was also a stale-truth hazard baked into a binary no text gate can read. Removed from the artwork entirely along with the dead `status:` field and `STATUS_COLOR` map; covers regenerated and re-verified on a fresh capture. A stray `Sparked` meta chip on Franchise Architect, visible only on the rendered image after the badge rule was already green, was removed and gated too.
- **Deferred one item honestly with the method attached:** the `postbuild` ordering audit needs an instrumented mtime-snapshot run, and the grep-based classification tried first was wrong in both directions. Doing it properly perturbs the build being converged for this deploy, so it returns to the board rather than shipping as a guess.
- **Ark cargo shipped, sibling tree untouched (CANON-018):** `pattern-share` to `*` on the verification-origin class, and a `repo-question` to studio-ops on a four-project status divergence between this repo's feeds and `PROJECT_REGISTRY.json`.
- **Deployed.** Pushed four commits directly to `main` after two publisher races (conflicts were all generated artifacts, resolved by taking one side and regenerating deterministically). Full production deploy run `33689612089` completed the release ceremony and promoted a clean dist; production serves `5cada45b6`, exactly the pushed HEAD. Twelve public routes verified 200 and three consolidated routes 301 with a real browser user-agent; the live cover PNG sha256-matches the regenerated local file.
- **The new staging gate proved itself in production.** It reported `staging-behind` right after the deploy (136 advertised routes vs 135, a news edition having landed mid-session) — a drift that would have passed silently before this session. Ran the remedy named on the artifact and parity returned `matched` 136/136, gating true.
- Receipts captured against the final tree after the last reseal: 84 hash-bound theme captures reviewed, mobile audit 215/215 passed.
...
- Final authority: `build:check` **283/283 EXIT 0** from step one (plan `cc6d6e067274aa90490eab13`, source `0a08fc8f1d753f7d442702d5`, receipt `3a68dcd75965141da58e4090`); unit **70/70**; accessibility **23/23 pre-rebase**, then **22 passed / 1 conditional portal-state skip** on the exact rebased bytes (the footer-label regression passed); CANON-053 News matrix **42/42** across three routes, seven themes, and desktop/mobile; changed structured data parse-clean; `~/.claude.json` valid; secret scans clean. Post-push CI exposed a real visible/unlabelled footer honeypot. It was fixed at `propagate-nav`, propagated to 113 pages, News pages were reconverged from their ledger, and the focused capture harness gained an exact requested-route receipt mode. The safe rebase also surfaced and the session fixed proof-chain drift, CANON-054/055 adoption, midnight-bound startup metadata, telemetry-ledger drift, null-timestamp honest-dark contract semantics, the required closeout cache boundary, and an orphaned visual-capture command. Release ceremony honestly rejected **4/8** on exact Obelisk redirect, staging browser receipt, promotion hold, and Doctor; Doctor remains `blockingFailing: 1` for deploy currency. The signed Obelisk dependency is now acknowledged, but remains incomplete; the callback registration still fails live.
...
- **Shipped 5/5** (`docs/AUDIT_2026-06-11-S189.{json,md}`, combined Priority 90.8):
  ...
  4. flagship-storytelling-wave2 — additive hero promise mirrored to franchise-architect (2nd live title / play-next destination).
...
## Session 308 — 2026-08-08 · editorial engine v2 · trend sourcing · The Dispatch

- Triage found S307 had closed out cleanly (all ten surfaces in tip `4da7eba13`, no lock, clean tree, write-back currency clean). No recovery commit was warranted and none was fabricated; the session synced 4 receipt-only commits and proceeded to the arc.
- Diagnosed the founder's persona question structurally: `direction` was a single scalar and `computeHeat` was mean pairwise distance on it, so the debate axis *was* hype level and every story produced the same argument shape. Adding a fourth hype opinion would have deepened the problem.
- Added a second stance axis `horizon` (-2 immediate … +2 structural) and made heat 2-D, plus `heatBreakdown()` naming the split shape. Backward compatibility is structural — `horizon` defaults to 0 and the divisor stays at the 1-D maximum — so published heat provably cannot move; asserted by test and confirmed by byte-stable ledger/carousel/claims/feed under `--check`.
- Expanded the cast by epistemic role rather than optimism: VERA (production practitioner), ECHO (cycle historian), JUNO (consequence desk), each with beats/lexicon/signature/forbidden/rival. Retained REX/MARA/DOT because the hash-chained ledger references their ids.
- Added `castForStory()` (deterministic beat-owning anchor + declared rival) so six voices produce rotation, not noise, and `personaForm()` so ledger accuracy becomes a writing directive — gated at four resolved calls so a thin record earns `unproven` and no tone shift.
- Added `EDITIONS` (Wire · Midday · Close · Late Night), moving volume discipline from per-day to per-edition; legacy days keep the 1–3 cap and half-editioned days are rejected.
- Shipped the trend radar (`news-trend-radar.mjs` + `lib/news-trends.mjs`, 56 self-tests): free key-less sources clustered into corroborated topics, corroboration weighted above engagement, with hard disqualifications for single-source rumour, re-runs, uncastable beats, and vendor marketing.
- Two bugs found by running the radar live rather than reading it: Google News links are `news.google.com` redirects, collapsing every outlet to one domain and silently killing the corroboration signal; and lab-blog case studies scored as news. Both root-fixed with regression tests. Live queue **7 → 24** with real multi-source corroboration.
- Shipped **The Dispatch**: Brevo list `3`, double-opt-in template `1`, `subscribe-desk-dispatch` deployed with `verify_jwt=false` pinned in `config.toml`, CTA on hub + story pages, `/news/subscribed/` landing. Deliberately identity-free because `send-member-newsletter` is Vault-Member-gated and The Desk promises no account. Live-verified **5/5** including negative controls; the probe contact correctly stayed off list 3 pending confirmation.
- CANON-053 caught a blocking light-theme contrast failure invisible in source review: a flat `background:var(--gold)` Subscribe button rendered near-black on dark amber because light-theme `--gold` (#7a5c00) is a text token. Fixed by reusing the sitewide `.button` gradient. 42 hash-bound captures, `blockingDefectsOpen: 0`.
- Root-fixed pre-existing genius-list debt: the generator marked BRAND items actionable while writing "requires founder sign-off" into their rationale, and the gate-integrity check correctly failed. Category-driven gating now makes generator and validator structurally unable to disagree.
...
### S308 final addendum — newsroom roles, working track record, story formats — 2026-08-08

- Three founder challenges, each exposing something structurally unbacked rather than merely missing.
- **"Do we have an editor/reviewer/publisher persona?"** No — all six were commentators. Checking surfaced a **P0**: `buildLedgerFromDays()` passed only `predictions`, never `resolutions`, so grading was erased on every `--rebuild`. `personaTrackRecords()` and `personaForm()` both derive from resolutions, so the public track record and the "publicly graded" line on every page could not work as built. Live state was ledger depth 1 / 4 predictions / **0 resolutions**, and the first prediction is not due until 2027-06-30 — it would have hidden for a year.
- Fixed: committed `data/news-desk/resolutions.json`, `planLedgerEntries()` attaching each grade deterministically (with a trailing entry so grading during a publishing gap cannot vanish), validation at the rebuild boundary, `--resolve` (append-only, refuses a grade without an evidence URL), `--record` (honest state + past-due warning). Both refusals verified live. **No resolution was fabricated to demo it.**
- Added **THE EDITOR / STANDARDS / CORRECTIONS** as roles distinct from commentators. `runStandards()` blocks a figure asserted in commentary that appears in no cited fact (a real hallucination proxy), uncited stances, foreign sources, and ungradeable predictions. `editorialReview()` runs or spikes with reasons and gates `--promote` — the refusal mechanism that makes safe autonomy reachable.
- **"Why is the grade about a year later?"** All four published predictions were 326–510 days out — individually falsifiable, collectively unfalsifiable on any timescale a reader checks. Standards now blocks an all-long-horizon story (≥1 call within 90 days); the drafter proposes a 45/120/240-day ladder instead of the flat 6-month default that would have perpetuated it. Published dates stand as written.
- **"Only long-graded predictions feels very robotic."** Correct and structural: every story required ≥2 facts, ≥2 stances, ≥1 dated prediction, so a sharp line about a viral moment could not publish without bolting a forecast onto the joke — and the radar filtered spectacle out as uncastable. Added six formats with per-format bars, a recurring column per persona, and a `spectacle` beat carried by ECHO/VERA. Facts stay real and cited in every format.
- Made the new machinery **visible**: the hub now renders the format board, the newsroom roles with what each refuses, each persona's column, and an honest record state that says plainly nothing has been graded yet. Rendered-pixel checked in dark/light/mobile.
...
## Session 338 — 2026-09-02 · arc: direct push to main + full production deploy authorized

**Intent:** run the full project-aware `/arc`, then commit and push directly to `main` and fully deploy production under founder authorization.

**Shipped:**
- Root-caused the red `uptime-probe` cron. Not the ordering race the board recorded: `observationFromReceipt()` never restored the three S336 content-clock fields, so every non-probe re-derive dropped them. Because `classify()` reads `contentLagHours`, the S336 content ceiling had been silently disabled on `build:check`, the content lane and every local build. Fixed, and the class closed with a fixed-point self-test that names no field plus a proof the guard can fail — 87/87, was 85 (D-S338.1).
- Fixed Lighthouse CI, red for 15 consecutive runs across ~27 hours. `/ranks/` and `/vaultsparked/` were retired behind `_redirects` 301s but still named as CI audit targets, including on the local preview, which has no edge layer. Removed from `lighthouse.yml`, `accessibility.yml` and `config/lighthouse-route-tiers.json`, and added `scripts/check-workflow-audit-targets.mjs` to `build:check` — two structural rules, neither naming a route (D-S338.2).
- Replaced the three-route staging parity sample with advertised-surface parity. Production advertises 134 routes, staging 115, 23 missing, staging `build-sha` `94e78e93` from 2026-08-28. Published as `surfaceParity` with `gating:false` and the reason on the artifact (D-S338.3).
...
## 2026-09-10 — Session 349 · arc: uptime observability honesty + edge-vantage sampler

- Ran the full arc: `/start` (S349, SIL 993 inherited), `/audit` (3 items, every premise pre-verified against live code and a live network probe), `/implement`, `/closeout`.
- **Found the session's real work by measurement, not from the board.** `data/uptime-history.ndjson` held 1488 retained rows and 604 of them read `edge-degraded`, every one with `contentOk: true` and `down: 0`. Probing directly: `GET /api/founder-presence.json` returns 200 from a residential IP and 403 in 127ms from CI. Cloudflare had widened bot challenges to JSON and OPTIONS paths, expiring the premise the probe was rewritten on in S177. `/status/` had therefore published a false outage continuously since 2026-07-13.
- Gave the `liveness`, `workerIngest` and `rumIngestPost` legs the challenge-shape classification the `login` leg already had, and introduced `observable` as a question separate from `ok`. A challenged leg is neither an outage nor a pass: `overall` reports the new `edge-unobservable`, unobserved rows leave the rollup denominators, and `dueAlerts` no longer pages on a challenge.
- Fixed a consequence nobody had noticed: `edgeHtmlBroken` guarded on `!liveness.ok`, which was permanently true while challenged, so the S179 apex-HTML failure shape it exists to catch could not have fired once in the whole window.
- Carried the new state to every consumer in the same session — `check-uptime-contract` STATES, the public `/status/` tile, rollup denominators, and the history row shape (`livenessObservable`, `workerIngestObservable`, `cv`).
- Shipped the Worker `scheduled()` uptime sampler DARK behind `UPTIME_SAMPLER_ENABLED="0"`: a Cloudflare cron is the only vantage that can observe our edge, because it is never challenged. Bounded KV (one fixed key per window, 7-day TTL — never a write per request, which is what exhausted the free tier in S319) and no subrequest to any hostname on our own routes (the S179 self-loop shape). Five unit tests assert the deploy is provably inert while the flag is off.
- Added `scripts/drain-uptime-kv.mjs` (12/12 self-tests): folds edge samples into the unchanged uptime contract, append-only, de-duplicated by timestamp, never rewriting an existing row and never interpolating a missing window.
- Taught the secrets gateway to consult the disproof it already held: `resolveCapability` returned READY for `supabase.admin` while the same map entry recorded `lastProbeStatus: "auth-error"`. It now renders a distinct FAILING state — which surfaced a second unknown failure, `openai.api` unreachable. Honest ready count 44/72, not 46/72.
- Separated the service worker page cache from the install precache. Both shared `CACHE_NAME`, and the LRU deletes `keys[0]` — the first entry ever written, which is `/` from `STATIC_ASSETS`. Past 60 entries every navigation was evicting a precached shell asset in order, quietly dismantling the offline experience. Also fixed `activate`, whose `k !== CACHE_NAME` test destroyed the API cache on every single activation.
...
### S357 — the three open board items, worked

**1. Desk comments P0 — diagnosed to the root, and it is not a credential.** Traced in layers: the studio gateway's `supabase.admin` is described as "self-hosted on Vorn" and its key decodes to `ref: ckwtolofoqzrqouqkmvs` ("VaultSpark Studios-2"), while the Worker calls `fjnpzjjyhnpmunfoycrp` ("VaultSpark Studios") — so the key was never invalid, it belonged to a different project. Fetching the right project's key via the Management API then revealed a second fault: the legacy JWT key type is retired there (the project carries `sb_publishable_`/`sb_secret_` issued 2026-03-12). And with the correct new-style secret applied to staging, the Worker still returned `comments_upstream_failed` — because of a third, decisive fact: **Supabase's own health API reports db, rest and auth ALL UNHEALTHY for that project ("Failed to connect to database") while its project-level status still reads `ACTIVE_HEALTHY`.** A provider outage wearing a healthy label. Nothing agent-side fixes it. Staging now holds the correct key; production still holds the wrong-project legacy one (the secret write was refused by the session's permission policy, and could not have been verified anyway while the project is down). `scripts/repair-desk-comments-credential.mjs` does the whole resolve-verify-apply safely — key never on disk, never printed, never in argv or env, piped to `wrangler secret put` on stdin — and refuses to claim `--provider-down` if the provider actually reports healthy. Self-test 5/5.

**2. The unhashed-script backlog — measured and now self-measuring.** `scripts/check-lane-held-asset-drift.mjs` (`npm run verify:lane-drift`) hashes every lane-held asset against the served copy. Live: 171 held, 137 match, 32 drift of which **19 still referenced**, 0 unknown. Two refinements it needed on first contact with reality: a transport failure or a 404 is not drift (reported separately, never folded into clean), and drift on an asset nothing loads is residue rather than risk — the first version flagged three scripts I had fingerprinted an hour earlier, because their bare names still appear in build-time bookkeeping. Reference detection is now the two shapes that reach a browser: a page's `<script src>` and a loader's `src:`. Self-test 9/9.

**3. The two promotion-receipt advisories now say something true.** "stranded/stale deploy" was firing on a healthy production because `promotedSha` is read from the repo's *current* build-sha rather than what was promoted; shell parity separates the cases, so it now distinguishes a genuinely stranded deploy (parity drift) from a promotion backlog (parity matched). And "7 console errors on the promoted artifact" could never reach zero: the probe runs at the Pages origin, which has no Worker, so Worker-only routes cannot succeed there — measured `/api/auth/me` apex 200 / pages.dev 404, apex console errors 0. Those are now counted in their own bucket and surfaced: "7 from Worker-only route(s) this Pages vantage cannot serve, 0 unexplained".


## S358 — 2026-09-18 — recovery, then every open S357 item closed
...
**2. "You asked" sourced from the changelog.** The live feed held "rebind the mobile proof without re-running the 215-cell audit", past all three S357 filters. `build-ship-receipts` now reads `data/consumer-changelog.json`, and only entries whose `answers` names the theme count. `publish-changelog-draft` accepts and validates `answers:`. The evidence graph input moved from commit-map to the changelog. Self-tests 24/24 and 9/9.

**3. /changelog/ vault_feedback 404.** PGRST205 means the table does not exist, and Q2 policy says it must not. Removed the fetch and the permanent "Aggregating…" note, made the stats static and true, dropped session codes, and removed the founder-presence entry for a feature retired in S356.
