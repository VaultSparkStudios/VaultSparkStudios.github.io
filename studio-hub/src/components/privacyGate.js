...
export function isUnlocked() {
  // On the hub subdomain, the Cloudflare Worker enforces real server-side
  // auth (see cloudflare/hub-auth.js). If the request reached the bundle at
  // all, the edge already validated the session cookie — skip the local
  // passphrase layer entirely to avoid a double prompt.
  try {
    if (typeof location !== 'undefined' && location.hostname === 'hub.vaultsparkstudios.com') return true;
  } catch {}
