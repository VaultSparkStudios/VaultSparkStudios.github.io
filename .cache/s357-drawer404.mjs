...
for (const [label, base] of [['STAGING','https://website.staging.vaultsparkstudios.com'],['PRODUCTION','https://vaultsparkstudios.com']]) {
  ...
  for (const t of THEMES) {
    const pill = p.locator(`.mobile-theme-pill[data-theme="${t}"]`);
    if (await pill.count()) { await pill.click(); await p.waitForTimeout(500); }
  }
  await p.waitForTimeout(2000);
  console.log(label, '→', bad.length, 'failed response(s)');
