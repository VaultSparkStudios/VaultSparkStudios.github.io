...
// Look local first (for projects that mirror it), then fall back to the
// sibling `vaultspark-studio-ops` repo checkout where the canonical copy lives.
...
for (const candidate of CANDIDATE_PATHS) {
  ...
  if (text) { content = text; sourcePath = candidate; break; }
}
...
if (jsonMode) {
  console.log(JSON.stringify({ genDate, ageDays, stale, critical, status, sourcePath }));
  process.exit(stale ? 1 : 0);
}

console.log(`${signal}  Revenue signals: ${genDate ?? 'unknown'} (${ageDays}d old) — ${status}`);
if (sourcePath) console.log(`   Source: ${path.relative(ROOT, sourcePath)}`);
if (stale) {
  console.log(`   Refresh in studio-ops: node scripts/render-revenue-signals.mjs`);
}
process.exit(stale ? 1 : 0);
