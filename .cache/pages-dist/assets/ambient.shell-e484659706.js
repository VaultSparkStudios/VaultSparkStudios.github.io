...
/* Sources: 31 files, parsed in declared order. */
...
(function () {
  ...
  function resolve(session) {
    ...
    document.body.setAttribute('data-vs-signed-in', signedIn ? 'true' : 'false');
    document.documentElement.setAttribute('data-vs-signed-in', signedIn ? 'true' : 'false');
    if (signedIn && session.tier) {
      document.body.setAttribute('data-vs-tier', session.tier);
      document.documentElement.setAttribute('data-vs-tier', session.tier);
    }
...
(function () {
  ...
  function mount(root, data) {
    style();
    root.innerHTML = '<div class="container"><div class="eyebrow">Feedback Operating Room</div><h2 style="font-family:Georgia,serif;font-size:clamp(1.8rem,3.5vw,2.8rem);margin:.4rem 0 1rem">Asked. Planned. Shipped. Explained.</h2><div class="fb-board__grid">' + (data.lanes || []).map(function (lane) {
...
(function () {
  ...
  function boot() {
    ...
    fetch('/api/social-dashboard-public.json', { cache: 'default' }).then(function (r) { return r.json(); }).then(function (data) {
      root.innerHTML = '<div class="container"><div class="eyebrow">Social Dashboard</div><h2 style="font-family:Georgia,serif;font-size:clamp(1.8rem,3.5vw,2.8rem);margin:.45rem 0 1rem">Every channel has a job.</h2><p style="color:var(--muted);max-width:70ch">' + esc(data.campaignFocus) + '</p><div class="social-live-grid" style="display:grid;grid-template-columns:repeat(3,1fr);gap:.8rem;margin-top:1.2rem">' + (data.channels || []).map(function (ch) {
        return '<a href="' + esc(ch.href) + '" target="_blank" rel="noreferrer me" style="display:block;border:1px solid var(--line);border-radius:14px;padding:1rem;background:rgba(255,255,255,.035);text-decoration:none;color:inherit"><span style="font-size:.72rem;text-transform:uppercase;letter-spacing:.1em;color:var(--gold);font-weight:800">' + esc(ch.label) + '</span><strong style="display:block;margin:.4rem 0">' + esc(ch.role) + '</strong><p style="color:var(--muted);font-size:.88rem">' + esc(ch.cadence) + '</p><em style="display:block;margin-top:.7rem;color:var(--dim);font-style:normal">' + esc(ch.nextAction) + ' →</em></a>';
      }).join('') + '</div></div>';
    }).catch(function () {});
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', boot);
...
(function () {
  ...
  function boot() {
    ...
    fetch('/api/security-posture.json', { cache: 'default' }).then(function (r) { return r.json(); }).then(function (data) {
      section.innerHTML = '<div class="eyebrow">Trust Center</div><h2 style="font-family:Georgia,serif;font-size:clamp(1.8rem,3.5vw,2.8rem);margin:.45rem 0 1rem">Public security posture.</h2><div style="display:grid;grid-template-columns:repeat(2,1fr);gap:.8rem">' + (data.controls || []).map(function (c) {
        return '<article style="border:1px solid var(--line);border-radius:14px;padding:1rem;background:rgba(255,255,255,.035)"><span style="font-size:.72rem;letter-spacing:.1em;text-transform:uppercase;color:var(--gold);font-weight:800">' + esc(c.status) + '</span><h3>' + esc(c.label) + '</h3><p style="color:var(--muted)">' + esc(c.detail) + '</p></article>';
      }).join('') + '</div>';
    });
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', boot);
