    ...
    // ── Kudos system ──────────────────────────────────────────────
    (function () {
      var _kudosLoaded = false;

      function initKudos() {
        if (_kudosLoaded) return;
        _kudosLoaded = true;

        var sendBtn  = document.getElementById('kudos-send-btn');
        var input    = document.getElementById('kudos-username');
        var feedback = document.getElementById('kudos-feedback');

        if (!sendBtn || !input) return;

        sendBtn.addEventListener('click', async function () {
          var handle = (input.value || '').trim().replace(/^@/, '');
          if (!handle) { setFeedback('Enter a Vault handle.', false); return; }
          sendBtn.disabled = true;
          sendBtn.textContent = 'Sending…';
          feedback.style.color = '';
          feedback.textContent = '';
          try {
            var { data, error } = await VSSupabase.rpc('send_kudos', { p_to_username: handle });
            if (error || data?.error) throw new Error((data?.error) || error?.message || 'Could not send kudos');
            setFeedback('Kudos sent to @' + handle + '! +2 XP for you ⚡', true);
            input.value = '';
            // Optimistic local XP bump
            if (typeof _currentMember !== 'undefined' && _currentMember) {
              _currentMember.points = (_currentMember.points || 0) + 2;
              var ptsEl = document.getElementById('stat-pts');
              if (ptsEl) ptsEl.textContent = _currentMember.points;
            }
            loadKudosReceived();
          } catch (err) {
            setFeedback(err.message || 'Failed to send kudos.', false);
          } finally {
            sendBtn.disabled = false;
            sendBtn.textContent = 'Send Kudos ⚡';
          }
        });
      ...
      function setFeedback(msg, ok) {
        var el = document.getElementById('kudos-feedback');
        if (!el) return;
        el.textContent = msg;
        el.style.color = ok ? '#10B981' : '#f87171';
      }

      async function loadKudosReceived() {
        try {
          var { data } = await VSSupabase.rpc('get_my_kudos_received');
          if (!data || !data.length) return;
          var section = document.getElementById('kudos-received-section');
          var list    = document.getElementById('kudos-received-list');
          if (!section || !list) return;
          section.style.display = '';
          list.innerHTML = data.slice(0, 5).map(function (k) {
            ...
            return '<div style="font-size:0.86rem;padding:0.35rem 0;border-bottom:1px solid rgba(255,255,255,0.04);display:flex;justify-content:space-between;gap:1rem;">'
              + '<span>⚡ <strong>@' + escHtml(k.from_username) + '</strong> sent you kudos</span>'
              + '<span style="color:var(--dim);flex-shrink:0;">' + label + '</span>'
              + '</div>';
          }).join('');
        } catch (e) {}
      }

      // Init when Following tab is first opened
      var _kudosTabBound = false;
      function bindKudosTabInit() {
        if (_kudosTabBound) return;
        _kudosTabBound = true;
        var tab = document.getElementById('tab-dash-following');
        if (tab) {
          tab.addEventListener('click', function () {
            setTimeout(initKudos, 80);
          });
        }
      }

      if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', bindKudosTabInit);
      } else {
        bindKudosTabInit();
      }
    })();