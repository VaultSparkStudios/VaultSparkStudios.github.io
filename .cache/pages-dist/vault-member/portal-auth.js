    ...
    // ── Auth error → friendly message map ───────────────────────
    function _mapAuthError(msg) {
      if (!msg) return 'Something went wrong. Please try again.';
      // Supabase v1/v2 credential errors
      if (/invalid.login.credential|invalid_credential/i.test(msg) || msg === 'Invalid login credentials') {
        return 'Incorrect email or password. If you forgot your password, use the "Forgot password?" link below.';
      }
      if (/email.not.confirmed|email_not_confirmed/i.test(msg) || msg === 'Email not confirmed') {
        return 'Please confirm your email before signing in. Check your inbox for the confirmation link.';
      }
      if (/user.already.registered|already.registered/i.test(msg)) {
        return 'An account with that email already exists. Try signing in instead.';
      }
      if (/duplicate.*username|username.*already.taken/i.test(msg)) {
        return 'That username is already taken. Please choose a different one.';
      }
      if (/captcha/i.test(msg)) {
        return 'CAPTCHA check failed. Please refresh the page and try again.';
      }
      if (/over.email.send.rate.limit|rate.limit|too.many.request/i.test(msg)) {
        return 'Too many attempts. Please wait a few minutes before trying again.';
      }
      if (/email.rate.limit/i.test(msg)) {
        return 'Too many sign-up attempts from this email. Please try again later.';
      }
      if (/weak.password|password.*at.least/i.test(msg)) {
        return 'Password is too weak. Please choose a stronger password (8+ characters recommended).';
      }
      if (/user.banned/i.test(msg)) {
        return 'Your account has been suspended. Please contact support.';
      }
      if (/network|fetch|failed to fetch/i.test(msg)) {
        return 'Connection error. Please check your internet connection and try again.';
      }
      // Return raw message only if it looks user-safe (no internal codes/stack traces)
      if (msg.length < 120 && !/[{}[\]]/g.test(msg)) return msg;
    ...
    document.getElementById('register-form').addEventListener('submit', async function(e) {
      ...
      try {
        ...
        if (authErr) throw new Error(_mapAuthError(authErr.message));
        ...
        if (rpcErr) throw new Error(_mapAuthError(rpcErr.message));
        if (rpcResult?.error) throw new Error(_mapAuthError(rpcResult.error));
    ...
    document.getElementById('login-form').addEventListener('submit', async function(e) {
      ...
      try {
        ...
        if (typeof VSMemberCache !== 'undefined') VSMemberCache.put(member);
