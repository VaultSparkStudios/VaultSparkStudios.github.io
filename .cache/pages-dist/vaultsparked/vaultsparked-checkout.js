...
(function () {
  ...
  async function loadPhaseData() {
    try {
      if (!VSPublic || typeof VSPublic.from !== 'function') return;
  ...
  async function startCheckout(plan, promoCode) {
    ...
    try {
      ...
      if (!client || !client.auth || !client.functions) {
        throw new Error('Checkout is loading. Please try again in a moment.');
      }
