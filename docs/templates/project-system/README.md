# Project System Templates

Use these templates when creating or migrating a VaultSpark project.

This public repo copy should be treated as a public-safe reference layer.
Sensitive template usage should happen in `vaultspark-studio-ops` or in private
project repos.

## Core

- `AGENTS_PROJECT.template.md`
- `PROJECT_BRIEF.template.md`
- `BRAIN.template.md`
- `SOUL.template.md`
- `CURRENT_STATE.template.md`
- `DECISIONS.template.md`
- `TASK_BOARD.template.md`
- `LATEST_HANDOFF.template.md`
- `WORK_LOG.template.md`
- `CREATIVE_DIRECTION_RECORD.template.md`
- `START_PROMPT.template.md`
- `CLOSEOUT_PROMPT.template.md`

## High-leverage memory

- `TRUTH_MAP.template.md`
- `OPEN_QUESTIONS.template.md`
- `ASSUMPTIONS_REGISTER.template.md`
- `RISK_REGISTER.template.md`
- `DECISION_HEATMAP.template.md`
- `LEARNING_LOOP.template.md`
- `INNOVATION_PIPELINE.template.md`
- `INNOVATION_PORTFOLIO.template.md`

## IP and brand protection

- `RIGHTS_PROVENANCE.template.md`
- `CANON.template.md`
- `FRANCHISE_BIBLE.template.md`
- `ENTITY_BIBLE.template.md`
- `RETCON_LOG.template.md`
- `CONTINUITY_RULES.template.md`
- `BRAND_SYSTEM.template.md`
- `SOCIAL_SYSTEM.template.md`
- `PROMISES_TO_AUDIENCE.template.md`
- `REFERENCE_WALL.template.md`
- `APPROVAL_MATRIX.template.md`

These templates are especially sensitive in real use and should generally be
filled in only in private repos.

## Operational support

- `PROMPT_LIBRARY.template.md`
- `ASSET_REGISTRY.template.md`
