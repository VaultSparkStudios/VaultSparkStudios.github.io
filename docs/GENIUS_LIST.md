...
#### 1. [PRODUCT] The live probe settles the dependency only on the no-cargo path. deri…
Final score: **96**
[S342][BUILD/P3] The live probe settles the dependency only on the no-cargo path. deriveDependency consults the probe inside if (!cargo). If the registration conversation is ever reopened, a found cargo takes the cargo path and DEMOTES a live-verified dependency from completed back to sent, re-raising releaseDependenciesSatisfied — the asking would undo the answer. Found mid-implementation when instructed to re-ship the request (D-S342.3); unfixed because the instruction was withdrawn and shipping it untested beside a deploy was the worse trade. Fix by applying liveSettles after the cargo-path status, with self-tests for both paths.
Why it matters: The live probe settles the dependency only on the no-cargo path. deriv is open, local, and unblocked — can ship this session.
...
#### 4. [VERIFY] resync-derived.mjs does not cover every --checked derived artifact, a…
Final score: **87**
...
Why it matters: The release-ceremony receipt truncates a failure message at 500 charac is a 5-session-old carry-forward; verify or close it so it stops polluting the hit list.
...
1. The live probe settles the dependency only on the no-cargo path. deri…
2. Post-push CI confirmation
3. The silent cron verdict is fixture-proven only. check-scheduled-workf…
4. resync-derived.mjs does not cover every --checked derived artifact, a…
