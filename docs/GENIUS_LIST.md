...
#### 1. [VERIFY] Complete release checks, push main, deploy production and verify serv…
Final score: **86**
Complete release checks, push main, deploy production and verify served repair.
Why it matters: Complete release checks, push main, deploy production and verify serve is a 364-session-old carry-forward; verify or close it so it stops polluting the hit list.

First command: `npm run build:check`

#### 2. [VERIFY] Verify one real confirmation only after explicit email-send authoriza…
Final score: **83**
...
#### 5. [PRODUCT] Mount the narrative on /journal/. Deferred: a UI change needs its own…
Final score: **72**
...
#### 1. [VERIFY] Compact the agent memory index by hand (21.3KB, target <17.1KB). An a…
Final score: **71**
...
5. Complete release checks, push main, deploy production and verify serv…
6. Verify one real confirmation only after explicit email-send authoriza…
7. Purge api/founder-presence.json from public git history. 30 revisions…
8. Staging Worker observability. Still the only way to source the S354/S…
9. Mount the narrative on /journal/. Deferred: a UI change needs its own…
10. Compact the agent memory index by hand (21.3KB, target <17.1KB). An a…
11. Enable Worker observability for staging. Deferred: needs a Worker dep…
12. Retire the 604 unresolved pre-S349 uptime rows on merit, not by rewri…
