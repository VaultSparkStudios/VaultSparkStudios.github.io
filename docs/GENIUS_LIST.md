...
#### 1. [PRODUCT] Console hygiene. View Transitions unhandled rejection (assets/native-…
Final score: **96**
[S302→NEXT][OBS/P1] Console hygiene. View Transitions unhandled rejection (assets/native-feel.js:22) — also eagerly boots Sentry on every affected page transition via sentry-init.js:39, burning quota on noise. Sentry sourcemap comment (vendored bundle line 3; rotates the content hash, cascading to sentry-init.js, its shell twin, sw.js, check-sri.mjs). Authorization already granted, so this is ordinary agent work.
Why it matters: Console hygiene. View Transitions unhandled rejection (assets/native-f is open, local, and unblocked — can ship this session.
...
#### 3. [PRODUCT] Trim the stale production-hold reasons. Three of five are resolved an…
Final score: **93**
[S302→NEXT][TRUTH/P1] Trim the stale production-hold reasons. Three of five are resolved and one exists in no script; api/release-proof.json publishes all five. Authorization already granted, so this is ordinary agent work.
Why it matters: Trim the stale production-hold reasons. Three of five are resolved and is open, local, and unblocked — can ship this session.
...
#### 5. [PRODUCT] Served-surface continuity registry. Generalize the S299 anchor+compar…
Final score: **69**
...
#### 1. [SECURITY] The token 400 silent sign-out
Final score: **99**
[S302→NEXT][UX/P0] The token 400 silent sign-out — the one user-visible bug, still unfixed. A member with a valid edge session is shown a signed-out portal, with no retry and no message. Root cause is the Worker handing the browser an already-rotated Supabase pair (cloudflare/obelisk-auth.js:539); the browser then calls clearCompatibilitySession (assets/supabase-client.js:122-126) and memoizes signed-out for the page load. Authorization already granted, so this is ordinary agent work: re-issue a fresh compatibility session at the edge, and stop failing silently in the browser. Planned, not shipped.
...
1. Console hygiene. View Transitions unhandled rejection (assets/native-…
2. Post-push CI confirmation
3. Trim the stale production-hold reasons. Three of five are resolved an…
4. context-meter.mjs publishes a false green. It reported "1.5% used · C…
5. Structured receipt on Obelisk link FAILURE. Today a failed link logs …
6. Break the agents.json build cycle. agents.json → proof-surface → stat…
7. Kill the login scan cliff with public.obelisk_identity_link
8. Wave D depth. /proof public in-browser verifier (the transparency app…
9. Served-surface continuity registry. Generalize the S299 anchor+compar…
10. The plan-inheritance fix cannot be proven end-to-end against live dat…
11. Ledger monotonicity tripwire. Persist the last-observed served ledger…
12. worker-route-provenance renders a Cloudflare bot-challenge as a route…
