...
- `npm run verify:all-pages`
  - **No-browser full-route crawl.** Starts the local preview server, fetches every HTML route in the repo, and fails on non-2xx/3xx responses or pages with more than 2 parser-blocking local scripts. This now runs inside `npm run build:check`.

## Codex MCP note

If `codex mcp list` shows no MCP servers in a subprocess, check the Codex home first. Sandbox-launched shells can default to `C:/Users/CodexSandboxOffline\.codex`, while the real Studio MCP config lives in `<user-home>\.codex`.

Use this verification recipe before re-registering servers:

```powershell
$env:CODEX_HOME='<user-home>\.codex'
codex mcp list
codex mcp doctor
```

Expected Studio entries: `studio-ops` and `ignis`.
...
- `extended` defaults to `--workers=1`
