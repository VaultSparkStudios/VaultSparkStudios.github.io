...
║  Session S338 · 2026-09-02 · agent: claude-code · repo: VaultSparkStudios.github.io           ║
╠═════════════════════════════════════════════════════════════════════════════════════════════╣
║                                                                                               ║
║  HEADLINE                                                                                     ║
...
╚═════════════════════════════════════════════════════════════════════════════════════════════╝
  ...
  [#1]  deploy-currency-round-trip-closed                         PROJ 10  ·  ECOS 8
         ── correctness ─────────────────────────────────────────────────────────────────────
         The board recorded the red uptime-probe cron as an ordering race; reproducing it
         first showed that fix would not have worked, because the receipt round trip itself
         was lossy. observationFromReceipt() never restored the three content-clock fields
         added in S336, the third field forgotten by the reader after retainedForHours in
         S300 and historyComplete in S316. classify() reads contentLagHours, so every
         non-probe re-derive collapsed it to null and the alarm S336 built to catch a
  ...
  [#2]  stranded-audit-targets-gated                              PROJ 9  ·  ECOS 8
         ── observability ───────────────────────────────────────────────────────────────────
         /ranks/ was consolidated into /leaderboards/#ranks and answered by a _redirects 301,
         but three CI audit-target lists still named it. One of them audits the local
         preview, which serves the built tree with no edge layer, so the redirect that makes
         production correct cannot apply there and the audit asked for a page that cannot
         exist. One 404 fails the whole job, so the site's performance gate produced no
  ...
  [#3]  staging-surface-parity-measured                           PROJ 8  ·  ECOS 7
         ── truth ───────────────────────────────────────────────────────────────────────────
         check-staging-parity compared a hand-maintained sample of three routes and was
         structurally incapable of noticing an absent one, which is why S337 could only
         record /how-we-build/ as prose. It now compares the surface each origin advertises,
         so coverage is complete and a new route is covered the moment it enters the sitemap.
         Production advertises 134 routes, staging 115, 23 missing, and staging's build-sha
  ...
  BLOCKERS
    ...
    • Trusted Types enforce remains held on load order: 31 sink-bearing assets load before ambient-core installs the default policy they depend on (D-S337.3).
    • Four public tables still have no anonymous read path; the fix is a founder privacy decision, not an implementation gap (D-S336.5).
