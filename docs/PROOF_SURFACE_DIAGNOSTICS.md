# Proof Surface Diagnostics

Generated: 2026-09-17T18:47:55.751Z
Receipt: `c19da02c493079c6a7f2cada` · coverage 88/109

Latest: **87/88** passed · blocking 87/88 · advisory findings 0/0 · total 23.6s

## Slowest Substeps

| Step | Class | Duration | Status | Command |
|---:|---|---:|---:|---|
| 49 | blocking | 0.8s | 0 | `node scripts/clean-stale-shells.mjs --check` |
| 78 | blocking | 0.6s | 0 | `node scripts/build-news-visual-receipts.mjs --check` |
| 35 | blocking | 0.6s | 0 | `node scripts/inject-breadcrumb-jsonld.mjs --check` |
| 46 | blocking | 0.6s | 0 | `node scripts/derive-game-nav.mjs --check` |
| 14 | blocking | 0.6s | 0 | `node scripts/build-og-cards.mjs --self-test` |
| 37 | blocking | 0.5s | 0 | `node scripts/build-news-desk.mjs --check` |
| 24 | blocking | 0.5s | 0 | `node scripts/check-game-playability-coherence.mjs` |
| 18 | blocking | 0.5s | 0 | `node scripts/check-videogame-schema.mjs` |
| 16 | blocking | 0.5s | 0 | `node scripts/inject-collection-jsonld.mjs --check` |
| 19 | blocking | 0.4s | 0 | `node scripts/enrich-videogame-schema.mjs --check` |

## Failures

- Step 88 [blocking]: `node scripts/check-receipt-ordering.mjs` exited 1 — self/contract
