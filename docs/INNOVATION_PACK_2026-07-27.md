...
| 21 | Isolated-checkout evidence semantics | Public CI lacks the private sibling revenue source, so re-resolving a locally rendered observation produced a false contradiction. | Source-present agreement stays strict; source-absent CI emits explicit SKIP/unverifiable, with a behavioral contract proving unavailable cannot pass. |

Outcome: twenty-one of twenty-one second-order candidates shipped. No public page changed, so the Lighthouse page-change gate is not applicable. No sibling repository was edited; both cross-repo requests used signed Ark cargo.