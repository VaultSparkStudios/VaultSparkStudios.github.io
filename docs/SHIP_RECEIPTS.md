<!-- generated-by: scripts/build-ship-receipts.mjs -->
<!-- generated-at: 2026-09-17 -->

# Ship Receipts

Public-safe receipts connecting aggregate feedback themes to shipped work and proof artifacts. Raw feedback text is intentionally absent.

| Theme | Signals | Shipped evidence | Visual proof |
|---|---:|---|---|
| Transparency | 3 | 729af46e the changelog timeline's two accents failed AA in light theme<br>171e2835 make the /changelog/ ship filter structural, not a word list | pending |
| Speed | 2 |  | home-lcp-s173 (4) |
| Front door | 4 |  | pending |
