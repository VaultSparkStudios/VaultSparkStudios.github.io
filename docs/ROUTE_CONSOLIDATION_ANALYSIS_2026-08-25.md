...
| Orphan batch | /notebook/, /ip/, /brand/system/, /ignis/roi/, and /security/trusted-types/ each had one inbound HTML file. Legacy /franchise-architect/ and /solara/ roots each appeared across 132 files because shell propagation carried old runtime links. /share/ is intentionally excluded because it is the manifest-declared Progressive Web App share target. | Notebook is now linked from Studio. Community and Vault Wall remain linked member surfaces. Rights, brand, IGNIS, security, and game marketing pages already contain the canonical destination content. The share receiver remains a distinct installed-app capability. | Link /notebook/, /community/, and /vault-wall/; preserve /share/; retire the remaining aliases with permanent redirects and noindex fallback stubs. |
