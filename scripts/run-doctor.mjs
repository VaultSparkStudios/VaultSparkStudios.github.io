...
const DRIFT_META = {
  ...
  'sibling-locks':       { driftClass: 'derived-stale', blocking: false },
...
const CHECKS = [
  ...
  },
  {
    ...
    parse: (out, code) => ({ pass: code === 0, detail: code === 0 ? 'all canons aligned' : firstLine(out) }),
  },
  {
  ...
  },
  {
    ...
    parse: (out, code) => ({ pass: code === 0, detail: code === 0 ? 'baseline holding' : firstLine(out) }),
  },
  {
    id:    'launch',
  ...
  {
    id:    'sibling-locks',
    label: 'Sibling session locks',
    cmd:   ['scripts/check-sibling-locks.mjs', '--json'],
    parse: (out, code) => {
      try {
        const start = String(out).indexOf('{');
        const jsonSlice = start >= 0 ? String(out).slice(start) : out;
        const d = JSON.parse(jsonSlice);
        if (!d.totalLocks) {
          return { pass: true, detail: 'no sibling locks detected' };
        }
        if (!d.staleCount) {
          return { pass: true, detail: `${d.totalLocks} active sibling lock(s), none stale` };
        }
        const oldest = Array.isArray(d.locks) ? d.locks[0] : null;
        const detail = oldest
          ? `${d.staleCount} stale / ${d.totalLocks} total · oldest ${oldest.repo} ${oldest.ageHours}h`
          : `${d.staleCount} stale sibling lock(s)`;
        return { pass: false, warn: true, detail };
      } catch {
        return { pass: code === 0, detail: 'lock scan unavailable' };
      }
    },
  },
  {
