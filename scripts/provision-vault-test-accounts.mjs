...
async function updatePlaywrightEnv(results) {
  ...
  const merged = {
    ...
    VAULT_ETERNAL_TEST_EMAIL: existing.VAULT_ETERNAL_TEST_EMAIL || '',
    VAULT_ETERNAL_TEST_PASSWORD: existing.VAULT_ETERNAL_TEST_PASSWORD || '',
  ...
  for (const result of results) {
    ...
    if (result.account.kind === 'eternal') {
      merged.VAULT_ETERNAL_TEST_EMAIL = result.account.email;
      merged.VAULT_ETERNAL_TEST_PASSWORD = result.account.password;
    }
  ...
  const content = [
    ...
    '',
    `VAULT_ETERNAL_TEST_EMAIL=${merged.VAULT_ETERNAL_TEST_EMAIL}`,
    `VAULT_ETERNAL_TEST_PASSWORD=${merged.VAULT_ETERNAL_TEST_PASSWORD}`,
    '',
