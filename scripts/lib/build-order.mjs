...
export const REFRESH_LIVE_DATA_ORDER = [
  ...
  { script: 'build-public-ecosystem.mjs', timeout: 30000, why: 'projects the refreshed public catalog into the canonical ecosystem feed' },
  { script: 'probe-canonical-destinations.mjs', timeout: 30000, why: 'samples destinations from the refreshed ecosystem feed' },
  { script: 'build-llms-full-shards.mjs', timeout: 60000, why: 'rebuilds agent discovery shards from the refreshed ecosystem feed' },
  ...
  { script: 'build-feedback-provenance.mjs', timeout: 30000, why: 'projects the refreshed commit map into feedback evidence' },
  { script: 'build-forge-feed.mjs', timeout: 30000, why: 'projects the refreshed commit map into deterministic JSON and RSS feeds' },
  ...
  { script: 'build-changelog-live.mjs', timeout: 30000, why: 'renders refreshed public-intelligence entries into the Changelog before other shared-output writers' },
  ...
  { script: 'build-news-critique-packets.mjs', timeout: 30000, why: 'joins published facts, arguments, predictions, and visual receipts into per-story critique packets' },
  { script: 'build-security-posture.mjs', timeout: 30000, why: 'refreshes the public security evidence leaf' },
  { script: 'build-identity-migration-receipt.mjs', timeout: 30000, why: 'recomputes identity evidence after control-plane refreshes' },
  ...
  { script: 'build-agents-json.mjs', timeout: 30000, why: 'publishes the refreshed ecosystem and shard inventory for agents' },
  { script: 'build-ai-discovery-health.mjs', timeout: 30000, why: 'verifies the refreshed agent discovery surface' },
...
function selfTest() {
  ...
  const asserts = [
    ...
    ['refresh public ecosystem precedes destination probe and shards', refresh.indexOf('build-public-ecosystem.mjs') < refresh.indexOf('probe-canonical-destinations.mjs') && refresh.indexOf('build-public-ecosystem.mjs') < refresh.indexOf('build-llms-full-shards.mjs')],
    ['refresh commit map precedes feedback and forge projections', refresh.indexOf('build-commit-map.mjs') < refresh.indexOf('build-feedback-provenance.mjs') && refresh.indexOf('build-commit-map.mjs') < refresh.indexOf('build-forge-feed.mjs')],
    ['refresh Changelog shared writers preserve live then receipt order', refresh.indexOf('build-changelog-live.mjs') < refresh.indexOf('build-you-asked-shipped.mjs')],
    ['refresh agents follow shards and precede candidate seal', refresh.indexOf('build-llms-full-shards.mjs') < refresh.indexOf('build-agents-json.mjs') && refresh.indexOf('build-agents-json.mjs') < refresh.indexOf('build-candidate-artifact-manifest.mjs')],
    ...
    ['refresh critique packets follow visual receipts', refresh.indexOf('build-news-visual-receipts.mjs') < refresh.indexOf('build-news-critique-packets.mjs')],
