...
// In production: prefer the local repo's secrets/ dir when present; fall back
// to the canonical secrets/ in the sibling vaultspark-studio-ops checkout.
// Public-safe repos (this one, IdeaForge, etc.) never carry their own copy,
// so without the fallback every capability resolves to "missing env" and
// gateway-driven scripts (probe-capability, check-secrets, doctor) misreport
// readiness. (S112 fix; restored S114 after S113 regression.)
const SECRETS_DIR = (() => {
  if (process.env.VAULTSPARK_SECRETS_DIR_OVERRIDE) return process.env.VAULTSPARK_SECRETS_DIR_OVERRIDE;
  const candidates = [
    path.join(REPO_ROOT, 'secrets'),
    path.resolve(REPO_ROOT, '..', 'vaultspark-studio-ops', 'secrets'),
  ];
  // Prefer the candidate that actually has CAPABILITY_MAP.json — a bare
  // local secrets/ directory often exists from prior audit() calls
  // (mkdirSync writes .access.log there) but carries no real content.
  const withMap = candidates.find(p => fs.existsSync(path.join(p, 'CAPABILITY_MAP.json')));
