...
function selectFeaturedAccounts(accounts) {
  // The "most popular" platforms — promoted to the social page hero.
  // Ordered roughly by reach + studio posting cadence.
  const featuredIds = [
    'youtube',
    'github',
    'reddit-community',
...
export async function buildPublicContracts(runtimePack, projectStatus, pulse, catalog, normalizedActivity = null) {
  ...
  const sharedActivity = normalizedActivity || {
    schemaVersion: '1.0',
  ...
  const websitePublic = {
    ...
    normalizedActivity: sharedActivity,
    socialPresence: {
