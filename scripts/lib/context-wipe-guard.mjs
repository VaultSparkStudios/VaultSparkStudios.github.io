...
// ── CLI / self-test (import-safe) ────────────────────────────────────────────
// Side effects run ONLY when executed directly (node context-wipe-guard.mjs …),
// never on import — see feedback_import_safe_side_effect_scripts.
const __isMain = (() => {
  try {
    const here = fileURLToPath(import.meta.url).replace(/\\/g, '/');
    const invoked = (process.argv[1] || '').replace(/\\/g, '/');
    return invoked && here.endsWith(invoked.split('/').pop());
  } catch { return false; }
})();
...
if (__isMain) {
  ...
  if (arg === '--self-test') {
    ...
    const { mkdtempSync, writeFileSync, rmSync, mkdirSync } = fsmod;
    let pass = 0, fail = 0;
    const ok = (cond, label) => { if (cond) { pass++; } else { fail++; console.error(`  ✗ ${label}`); } };
    ok(appendOnlyPreserved('A\nB\nC', 'A\nB\nC\nD'), 'pure append preserved');
    ok(appendOnlyPreserved('# H\n\n## old', '# H\n\n## new\n\n## old'), 'prepend-after-header preserved');
    ok(!appendOnlyPreserved('A\nMIDDLE\nC', 'A\nEDITED\nC'), 'mid-edit detected as violation');
    ok(!appendOnlyPreserved('A\nB\nC\nD', 'A\nD'), 'deletion detected as violation');
    ok(appendOnlyPreserved('', 'anything'), 'empty prior preserved');
    const withRoll = '# H\n<!-- rolling-status-start -->OLD<!-- rolling-status-end -->\n## entry';
    const newRoll  = '# H\n<!-- rolling-status-start -->NEW<!-- rolling-status-end -->\n## entry';
    ok(appendOnlyPreserved(withRoll, newRoll), 'rolling-status refresh not flagged');
    ok(isGeneratedWiped(''), 'empty generated == wiped');
    ok(!isGeneratedWiped('Generated: x\n## Real entry'), 'valid small regen not wiped');
    ok(isGeneratedWiped('- active item:\n'), 'template scaffold == wiped');
    const dir = mkdtempSync(join(process.env.TEMP || process.env.TMPDIR || '/tmp', 'cwg-'));
    try {
      const f = join(dir, 'note.md');
      writeFileSync(f, 'x'.repeat(1000));
      let threw = false;
      try { assertSafeWrite(f, 'x'.repeat(100)); } catch { threw = true; }
      ok(threw, 'assertSafeWrite throws on >50% shrink');
      threw = false;
      try { assertSafeWrite(f, 'x'.repeat(900)); } catch { threw = true; }
      ok(!threw, 'assertSafeWrite allows minor shrink');
      mkdirSync(join(dir, 'context'), { recursive: true });
      const handoff = join(dir, 'context', 'LATEST_HANDOFF.md');
      writeFileSync(handoff, 'y'.repeat(1000));
      threw = false;
      try { assertSafeWrite(handoff, 'y'.repeat(10)); } catch { threw = true; }
      ok(!threw, 'SHRINK_ALLOWED file may shrink');
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
    console.log(`context-wipe-guard --self-test: ${pass} passed, ${fail} failed`);
    process.exit(fail === 0 ? 0 : 1);
  } else if (arg === '--check') {
    const root = process.cwd();
    const { ok: clean, findings } = checkContextFiles(root);
    if (clean) {
      console.log('context-wipe-guard --check: ✓ no context-wipe findings');
      process.exit(0);
    }
    console.error('context-wipe-guard --check: ⚠ findings:');
    for (const f of findings) {
      console.error(`  ${f.issue}  ${f.file}  (${(f.ratio * 100).toFixed(1)}% of HEAD)`);
    }
    process.exit(1);
  } else {
    console.log('context-wipe-guard — usage: --self-test | --check');
    process.exit(0);
  }
}
