...
export function mergeRawRumFileLists(...groups) {
...
if (process.argv.includes('--self-test')) {
  ...
  const checks = [
    ...
    ['tracked and freshly downloaded raw files are unioned', JSON.stringify(mergeRawRumFileLists(['old.json'], ['new.json', 'old.json'])) === JSON.stringify(['new.json', 'old.json'])],
  ];
  let pass = 0;
...
function listRawRumFiles() {
  ...
  try {
    ...
    for (const file of tracked) trackedFiles.push(path.join(ROOT, file));
  } catch { /* fall back to filesystem traversal */ }

  const discoveredFiles = [];
  if (!fs.existsSync(RAW_DIR)) return mergeRawRumFileLists(trackedFiles);
  for (const day of fs.readdirSync(RAW_DIR).sort()) {
    const dir = path.join(RAW_DIR, day);
    if (!fs.statSync(dir).isDirectory()) continue;
    for (const f of fs.readdirSync(dir).sort()) {
      if (f.endsWith('.json')) discoveredFiles.push(path.join(dir, f));
    }
  }
  return mergeRawRumFileLists(trackedFiles, discoveredFiles);
}
