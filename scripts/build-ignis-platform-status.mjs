...
if(rich&&rich.schema==='ignis-platform-public@1'){
  ...
  fs.writeFileSync(OUT,JSON.stringify({...rich,_builtAt:new Date().toISOString(),_sourceRepo:'vaultspark-ignis',_sourceFile:'ignis/output/ignis-platform-public.json'},null,2));
...
fs.writeFileSync(OUT,JSON.stringify({schema:'ignis-platform-public@1',generatedAt:new Date().toISOString(),status:'unknown',statusLabel:'Data unavailable',sil:{score:null,max:1000,pct:null,trend:'→',sparkline:[]},intelligence:{health:null,healthMax:100,healthPct:null},sessions:{total:null},capabilities:[],voice:'IGNIS platform data is temporarily unavailable.',_placeholder:true},null,2));
