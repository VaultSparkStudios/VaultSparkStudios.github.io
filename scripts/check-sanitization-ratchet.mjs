...
function resolveSummaryPath() {
  ...
  if (!fs.existsSync(auditsBase)) {
    console.log('No audits/sanitization/ directory yet — ratchet inactive until first scan is written.');
    process.exit(0);
  }
  ...
  if (!entries.length) {
    console.log('No dated audit directories in audits/sanitization/ — ratchet inactive until first scan is written.');
    process.exit(0);
  }
