...
// S120: NONCE_CSP_ENABLED=1 is active in the Cloudflare Worker.
// Meta CSP tags have been stripped from all HTML files (scripts/strip-meta-csp.mjs).
// Running this script would re-add meta CSP tags, conflicting with the nonce HTTP header.
// This script is intentionally disabled while nonce mode is active.
if (process.argv.includes('--dry-run') || process.argv.includes('--check-skipped')) {
  console.log('propagate-csp.mjs is DISABLED — nonce-based CSP is active (NONCE_CSP_ENABLED=1).');
  console.log('Meta CSP tags have been stripped. No propagation drift to check.');
  process.exit(0);
}
console.error('propagate-csp.mjs is DISABLED — nonce-based CSP is active (NONCE_CSP_ENABLED=1).');
console.error('Meta CSP tags have been stripped. Do not re-add them.');
console.error('To update the CSP hash list, edit config/csp-policy.mjs + redeploy the Worker.');
process.exit(1);

const CSP_VALUE = PAGE_CSP;
