...
// REVENUE_SIGNALS.md is portfolio-level intelligence owned by studio-ops.
// Mirror check-revenue-freshness.mjs's resolution order: local first (for
// projects that keep a mirror), then the sibling studio-ops checkout where
// the canonical copy lives. Without this fallback the brief reports the
// revenue signal as "not found" even when the sibling file is fresh.
const revSig      = readText(path.join(root, 'portfolio', 'REVENUE_SIGNALS.md')) ||
                    readText(path.resolve(root, '..', 'vaultspark-studio-ops', 'portfolio', 'REVENUE_SIGNALS.md'));
...
// ── IGNIS spend (P0 Token Governance) ────────────────────────────────────────
// Reads .cache/ignis-spend.json written by `scripts/check-ignis-spend.mjs`.
// Fail-soft: if cache absent, line shows "?" so brief never breaks on this.
let ignisSpend = null;
try {
  const cachePath = path.join(root, '.cache', 'ignis-spend.json');
...
const lines = [
  ...
  row(`${sigSpend}  IGNIS spend   ${spendDetail}`),
  bot(),
  ``,
  // ── IGNIS INSIGHT ──────────────────────────────────────────────────────────
