...
function buildDirectorsReportPage() {
  ...
  const head = chromeHead({
    ...
    canonical: url,
    // Bespoke share card rendered by build-news-desk's rasterizeDirectorsCard().
    // build-og-cards skips news pages because their OG image paths (/assets/og/news/...)
    // are not in the /assets/og/og-*.png pattern that isOurs() matches.
