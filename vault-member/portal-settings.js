    ...
    (async function init() {
      ...
      if (session) {
        // Instant pre-render: show cached member immediately while fresh data loads
        if (typeof VSMemberCache !== 'undefined') {
          const cached = await VSMemberCache.get(session.user.id);
        ...
        if (row) {
          ...
          const freshMember = buildMember(session.user, row);
          showDashboard(freshMember);
          if (typeof VSMemberCache !== 'undefined') VSMemberCache.put(freshMember);
