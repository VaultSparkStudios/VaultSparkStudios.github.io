    ...
    function renderAchievementsGrid(member, achEarned, unlockDates) {
      const grid = document.getElementById('achievement-grid');
      if (!grid) return;
      grid.innerHTML = '';
      const dateMap = unlockDates || {};
      VS.ACHIEVEMENT_DEFS.forEach(def => {
        ...
        const achIconHtml = def.icon && def.icon.startsWith('/')
          ...
          : def.icon;
        const unlockDate = earned && dateMap[def.id]
          ? new Date(dateMap[def.id]).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })
          : null;
        const unlockDateHtml = unlockDate
          ? `<div style="margin-top:0.3rem;font-size:0.68rem;color:var(--dim);">Unlocked ${unlockDate}</div>`
          : '';
        const tooltipText = earned
          ? `${def.desc}${unlockDate ? ' · Unlocked ' + unlockDate : ''}`
