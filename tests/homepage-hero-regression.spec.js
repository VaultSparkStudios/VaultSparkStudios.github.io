...
test('homepage stays prove-first (S123) — no ask-surface drift on /', async ({ page }) => {
  test.setTimeout(20000);
  await page.goto(BASE + '/', { waitUntil: 'domcontentloaded' });

  const drift = await page.evaluate(() => ({
    microFeedbackRoot: !!document.querySelector('[data-micro-feedback-root]'),
    dispatchStrip: !!document.querySelector('.dispatch-strip'),
    homePersonalizedWelcome: !!document.querySelector('.home-personalized-welcome, #home-personalized-welcome'),
  }));

  expect(drift.microFeedbackRoot).toBe(false);
  expect(drift.dispatchStrip).toBe(false);
  expect(drift.homePersonalizedWelcome).toBe(false);
});
