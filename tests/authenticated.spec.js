...
async function dismissWhatsNewModal(page) {
  // First-run tour and release notes can arrive in sequence. Exercise their
  // actual close controls before interacting with dashboard panes beneath.
...
test.describe('Vault portal authenticated flows', () => {
  ...
  test('device theme override persists into vault status messaging', async ({ page, request }) => {
    ...
    await expect(page.locator('#vault-status-theme')).toContainText(/local (?:override, account backup|\+ account)/);
    await expect(page.locator('.theme-tile[data-theme="lava"]')).toHaveClass(/active/);
  });
  ...
  test('account-backed theme sync restores after local preference is cleared', async ({ page, request }) => {
    ...
    await page.locator('.theme-tile[data-theme="cool"]').click();
    ...
    await dismissWhatsNewModal(page);
    await page.locator('#tab-dash-settings').click();
    await expect(page.locator('.theme-tile[data-theme="cool"]')).toHaveClass(/active/);
    await expect(page.locator('#vault-status-theme')).toContainText('Cool');
    await expect(page.locator('#vault-status-theme')).toContainText('local + account');
    await expect.poll(() => page.evaluate(() => localStorage.getItem('vs_theme'))).toBe('cool');
  });
  ...
  test('free and VaultSparked members diverge on archive access and live-tool entitlement', async ({ page, request }) => {
    ...
    await page.waitForFunction(() => Array.isArray(window._archiveFiles)
      || document.getElementById('archive-file-list')?.textContent?.includes('Could not load files.'));
    if (!await page.evaluate(() => Array.isArray(window._archiveFiles))) {
      const probe = await page.evaluate(async () => {
        const { data, error } = await window.VSSupabase.rpc('get_classified_files');
        return {
      ...
      expect(probe.error, `classified archive RPC failed: ${JSON.stringify(probe)}`).toBeNull();
      expect(probe.count, 'classified archive RPC returned no files').toBeGreaterThan(0);
    }
