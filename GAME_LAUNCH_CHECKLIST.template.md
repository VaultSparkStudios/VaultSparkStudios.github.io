...
- [ ] `deploy-pages.yml` is configured with the correct `__GAME_SLUG__`
- [ ] Optional: repo variable `GAME_SERVICE_ORIGIN` is set (if game has a backend)
- [ ] Optional: repo variable `API_DOMAIN` is set (if game has a backend)
