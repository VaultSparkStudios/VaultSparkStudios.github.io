import { getDefaultCareerRealismProfile, getDefaultRealismProfile } from "./realismProfiles.js";
...
export function loadRealismProfile() {
  return getDefaultRealismProfile();
}

export function loadCareerRealismProfile() {
  return getDefaultCareerRealismProfile();
}

export function applySeasonRealismCalibration({ league, year, profile = getDefaultRealismProfile() }) {
...
export function buildWeightedProfileFromPfrRows(
  rows,
  { fromYear = 2019, toYear = 2025, halfLife = 2.2, fallbackProfile = getDefaultRealismProfile() } = {}
) {
...
export function buildCareerProfileFromPfrRows(
  rows,
  {
    ...
    minCareerSeasons = 2,
    minCareerGames = 40,
    fallbackProfile = getDefaultCareerRealismProfile()
  } = {}
) {
  const profile = JSON.parse(JSON.stringify(fallbackProfile));
...
export function buildCareerCalibrationSnapshot({
  league,
  profile = getDefaultCareerRealismProfile(),
  includeActive = true
} = {}) {
...
export function buildPositionCalibrationSnapshot({ league, year, profile = getDefaultRealismProfile() }) {
  const snapshot = {};
  for (const [position, positionProfile] of Object.entries(profile.positions)) {
