...
1. **Confirm the recovery push landed and CI is green** — `gh run list --commit <recover-sha>`. The e2e compliance job is the one that exercises the Lighthouse volatility policy (D-S283.5) and the oracle public-feed contract (D-S283.3) end-to-end. If Lighthouse CI is red on `/`, read it through the NEW shared policy before treating it as real — that gate now corroborates single-run dips against the committed trend.
