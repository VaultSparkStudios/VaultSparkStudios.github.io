...
### S363 addendum — released, and verified from served bytes

**Pushed** `8aaf100c8` after two publisher rebases. Both were the hourly `[skip ci]` publishers racing the push, and both landed 17 and 7 generated-artifact conflicts respectively. Resolving a conflict only unblocks the rebase, so every derived artifact was regenerated from source afterwards (`npm run build` + `resync-derived`, 53 then 42 artifacts) rather than left at whichever side won. The pre-push coherence gate caught that distinction on the second pass: resolving `candidate-artifact-manifest.json` settled the file but left the graph it anchors inconsistent, and converging the graph — rather than resealing the one artifact the error named — was what made the gate true rather than merely green.

**Both proof receipts were recaptured, not rebound.** The rebase changed `index.html`, and `rebind-mobile-receipt` correctly refused: a rebind is for candidate-sha metadata drift, and this was real content. So the 84-capture theme matrix and the 215-cell mobile audit were both re-run against the final tree and re-bound at candidate `fc5471acf727`.

**Released.** Staging published 7,018 files (receipt `ef30e3790de4c9f84aff9c6b`); the staging release gate passed **3 passed · 3 held-by-design · 0 failed** — the held three are the identity contracts the scoped promotion deliberately does not ship, per the `PRODUCTION_PROMOTION.json` blast radius. Scoped promotion run `35541452760` succeeded, `deployedBy: pages-deploy`.

**Verified from served production bytes, not from the run's colour.** The deployed sha `cf48e380c` is not this session's HEAD — two automation commits landed on top — so it was checked by ancestry: `8aaf100c8` is an ancestor of the deployed commit, so S363 is contained in what is live. The one public-facing change proves itself on the wire: `/api/supabase-control-plane.json` now serves `overall: "partial"` with `managementApi`, `sqlMigration` and `edgeFunctions` all `ready` and the real blocker still named, and `/api/identity-migration-receipt.json` serves `{"overall":"partial","readyPlanes":3,"totalPlanes":4}` — internally consistent for the first time. `smoke-live` 6/6.

**Two gates found during the release, both fixed at source, `--allow-wipe` never used:** the context-wipe guard calling a file that had *grown* a content wipe (D-S363.7), and the ledger size gate's prescribed repair producing a tree that same guard rejected (D-S363.8). A third was recorded rather than fixed: the autopilot clears the session lock, after which the context meter cannot identify the agent, defaults to a 200 k limit and reports 234 k tokens as both 1 % and 117 %. The lock was restored because the session was genuinely still running; the ordering defect is left for the next session with this note as its evidence.

### Session 363 addendum — the third publisher rebase, and the phrase this record has to carry
