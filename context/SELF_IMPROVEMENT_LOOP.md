...
## 2026-09-07 — Session 345 (a repair tool reported clean over a subset it could not see, for two sessions and two hand-fixes · and a detector whose clean result was also evidence it had never been exercised) | Total: 984/1000 (v3.0) | Velocity: -2 | Debt: ↓
Avgs — 3: 984.0 | 5: 984.6 | 10: 988.0 (carried) | 25: 988.0 (carried, spans archive) | all: 984.0 (carried, spans archive)
Sparkline (last 5): █████ · 994, 977, 982, 986, 984

| Category | Score | Notes |
|---|---:|---|
| Dev Health | 99 | Every touched surface self-tested green: `resync-derived` 16/16 · `check-scheduled-workflow-staleness` 18/18 · `check-evidence-graph-coverage` 7/7 · doctor blockingFailing 0. The headline fix was proven by reproducing the original incident, not by a green on the fixed tree: with `api/intelligence-budget.json` staled, the old path exits 0 saying "5 artifact(s) rebuilt + staged" and the new one exits **1** naming the drifter. Deduction: the sweep has not yet run against a real publisher-race rebase in CI, which is its actual habitat. |
| Creative Alignment | 97 | Honest deduction — this session shipped no audience-facing change. It is repair and truth work behind the site, serving SOUL only indirectly (a Desk that publishes a non-edition, and a board that carries a false sentence, erode the same thing). Scored for what a visitor got, which is nothing new. |
| Momentum | 98 | Closed a carry open since S340 that had already cost two sessions of hand-fixing the same class. Also corrected a blocker sentence that would otherwise have spent founder attention on a problem that had already moved. Deduction: the newsletter — the single item the founder is actually waiting on — moved not at all. |
| Engagement | 96 | The Monthly Member Newsletter still sends nothing, for the seventh month. Re-probed rather than assumed: both faults are live, the credential path is open, the tooling passes 5/5 — and the deploy was denied by the sandbox classifier for the second session running. Scored on outcome, not effort. No new engagement surface shipped. |
| Process Quality | 98 | Refused three shortcuts, each of which would have produced a green: did not guess `sources` for the 38 unmodeled nodes (a confidently wrong graph is worse than an admittedly partial one, and the ratchet already said so); did not disable a live workflow to manufacture a `silent` cron to pin the fixture-only verdict against; did not re-run the denied deploy through a different shell, which would have been working around the denial rather than clearing it. Re-probed both carried blockers before repeating them, and one had expired. **Deduction, and it is mine:** I read the negative control's exit code through a pipe and reported `exit=0` for a run that had actually exited 1 — the exact failure my own notes warn about — and caught it only because the printed output contradicted the code. |
| Cross-Repo Coherence | 100 | No sibling repo written. The Supabase gateway half stays studio-ops' via the existing S344 Ark cargo. |
| Security Posture | 99 | `--sweep-repair` rebuilds only after the same world-acting-builder prefix test the graph path uses, so the new repair path cannot invoke a `deploy-`/`publish-`/`promote-`/`send-` script to make a file look right after a rebase — the hazard that rule was originally written for. The permission denial was reported, not routed around. |
| Ecosystem Integration | 99 | Reused rather than invented: the sweep's generator set comes from `check-evidence-graph-coverage`'s own enumeration, so the repairer and the ratchet can never disagree about what is covered — a second private list would have been the obvious and wrong move. Import-safety used the repo's established `isDirect` idiom rather than a new one. No new dependency, no new config surface. |
| Capital Efficiency | 100 | Zero new dependencies. The sweep is ~2 minutes of read-only `--check`s, run only on the rebase path, against a CI red that costs ten. |
| Automation Coverage | 98 | The sweep sits inside `resync-derived`, confirmed wired into `build:check`, and a **structural** self-test asserts that no bare `process.exit(0)` in `main()` can bypass it — so a future exit path cannot silently reintroduce the blindness, which is how this defect survived two sessions. `fixtureOnlyVerdicts` now prints on every staleness run. Deduction: 38 of 67 byte-checked generators remain unmodeled, so the graph still cannot topologically order them; the sweep makes that safe, not closed. |

**Top win:** A repair tool that reported clean over a subset it could not see. `resync-derived` walks an evidence graph modeling **29 of 67** byte-checked generators and printed `17 artifacts rebuilt + staged`, which reads as completeness. CI then failed ten minutes later on `build-intelligence-budget` (S340, run `33702593208` step 185) and on `build-nervous-system` (S341). Both were hand-fixed; both are still unmodeled today.

**The fix is not the one the item asked for, and that is the point.** The item said to derive the tool's set from what `build:check` gates. Modeling those 38 nodes requires their real `sources`, and `check-evidence-graph-coverage`'s own header already explains why guessing them is *strictly worse* than omitting them: `resync-derived` would rebuild in a wrong topological order, and the cascade checker would start demanding the wrong things of every cron. The ratchet is deliberate. So the fix ran at the other end — the tool now runs the unmodeled generators' own `--check`, which is a **measurement rather than a prediction**, and therefore needs none of the information the ratchet refuses to guess. Default is fail-named; repair is opt-in.

**Second win — a zero that could not distinguish health from blindness.** `check-scheduled-workflow-staleness`'s `silent` verdict has never fired on a live cron; it is proven by fixtures alone. `silent: 0` read identically whether the detector worked or was quietly broken. The run now computes `liveCorroboration` per verdict class and prints `fixtureOnlyVerdicts` on both the pass and the fail paths, so an untested-in-production code path is a declared one rather than an assumed-good one.

**Second gap — I re-learned a lesson I had already written down.** Verifying the negative control, I read the exit code through a pipe and reported `exit=0` for a run that had exited **1**. My own notes say never to read a verdict through a pipe. The reading was caught only because the tool's printed output contradicted it — that is luck, not method, and it is the same shape as the defect I spent the session fixing: a true-looking result whose scope nobody stated.

**Honesty ledger:** The newsletter is **still not deployed** and will fail again on 2026-10-02; the permission denial was hit a second time and I did not clear it. The Desk cadence escalation is **not resolved** — freshness reads `daily` again, but I could not measure whether the queue widened or the day was simply lucky, because the radar cache is CI-only and absent from a local tree, and 2 editions against a 4-slot/day promise is a partial recovery; the row now says exactly that rather than claiming a fix. The sweep is verified by self-test, a reproduced negative control and local runs — **not** by a live publisher-race rebase, which is the situation it exists for. 38 generators remain outside the graph. The QA Phase 0 human walkthrough, the Obelisk ceremony and the gateway slot are all untouched, as they have been for several sessions.

**Intent outcome:** Achieved for the arc. Two multi-session carries closed at the root, one carried blocker corrected rather than repeated, one blocker honestly re-escalated with the denial named — and the founder's deploy authorisation carried out.
