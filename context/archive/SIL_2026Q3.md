...
<!-- rotated-from: context/SELF_IMPROVEMENT_LOOP.md · 2026-09-07 -->
...
**Top gap:** Three separate measurements this session were wrong before they were right, all of them mine, all in the same shape: I inferred a property instead of observing it. A regex I wrote to check my own fix; a CSS selector that did not match the element; a computed-style walk for the effective background that bottomed out at white and gave the broken light theme a comfortable pass. The last one is the sharpest, because it *agreed with the fixed dark theme* — a reading that confirms what you expect on the case that works and lies about the case that does not. I only got a true number by sampling pixels out of a screenshot. My own rule for this exists and I reached it late: when a computed reading disagrees with the image, believe the image.

**Third win — the surface I fixed had never rendered.** Doing the CANON-053 pixel check on the changelog fix surfaced that `returning-signal-strip.js` has never once appeared in production: `returning-visitor-digest.js` loads first and stamps `vs_last_visit_ts` to `now` before the strip reads the same key. Same class as the S343 `vs_visit_count` fix, same two files, neighbouring key, missed then. Underneath it were two more defects nobody could have seen — `--vs-text` defined nowhere, and brand gold at 1.31:1 on the light theme's cream — because a surface that never renders is invisible to the theme matrix and the visual receipt too. 7/7 themes now clear AA on all four elements.
...
<!-- rotated-from: context/SELF_IMPROVEMENT_LOOP.md · 2026-09-17 -->
