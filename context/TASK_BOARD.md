...
- [x] **[S308][NEWS/P0][DEFECT FIXED] Resolutions were structurally discarded, so the track record could never work.** `buildLedgerFromDays()` passed only `predictions`, erasing every grade on rebuild — while each page claimed predictions are "publicly graded". Fixed with a committed resolutions source, deterministic re-attachment, and rebuild-boundary validation; a self-test asserts grading survives (it fails against the old code). D-S308.17.
