...
- [x] **[S308][RELEASE/P0] Root-cause the content-lane dispatch failure instead of retrying it.** The first `confirm_content` dispatch failed with `no baseline sha available`: `api/deploy-currency.json` carries `deployedSha: null` / `state: unobserved` because the production probe is Cloudflare-challenge-bound, so the lane could not compute its diff range. Re-dispatched with the authoritative value read from production itself (`/api/build-sha.json` → `4a72961d`), which is what the `baseline_sha` input exists for.
...
- [x] **[S308][TRUTH/P0] Harden authorship disclosure so no reader can mistake The Desk for human writing.** Founder directive. Audit found five gaps: the social card said "AI SIGNAL" (reads as the *topic*, since The Desk reports on AI) and attributed quotes like a columnist byline; the JSON Feed author was an ordinary masthead; feed items carried no author at all and syndicate alone; "experimental" appeared nowhere; and persona bylines were unlabelled in-page. `check-ai-disclosure-alignment.mjs` only covers /privacy and /terms and never looked at /news — which is how it shipped unguarded. Now enforced at five layers: pre-content banner, "AI persona" at every attribution point, JSON-LD `Organization` author (never `Person`) + `creditText`, feed author + per-item authors + machine-readable `_vaultspark_disclosure`, and "WRITTEN BY AI" on the card. Confirmation email leads with it.
- [x] **[S308][TRUTH/P0] Gate the disclosure so it cannot regress.** New `check-news-ai-disclosure.mjs` (17 self-tests, wired into build:check), mutation-tested red on the exact ambiguous "AI SIGNAL" wording and green on restore. Its own first version was caught cheating: it read the renderer's *source*, where the byline is the template literal `— ${persona.name}`, so the attribution rule could never match — green while proving nothing. It now renders a real card and asserts what a reader sees.
- [x] **[S308][NEWS/P1] Bridge the radar to an authored edition.** `news-draft-edition.mjs` (30 self-tests) fills every deterministic field — cast via `castForStory`, standing via `personaForm`, sourced fact candidates with citations, edition slot, prediction ids and `resolveBy` — and leaves only judgment fields blank. `--status` names the blanks; `--promote` fails closed on any blank or `validateDay()` error. **Zero model calls**: CANON-015 puts authoring on the Max Plan surface, not a metered API call.
- [x] **[S308][NEWS/P0] Fix a self-inflicted honesty bug in the drafter.** Its first live run reported 4 "ok" sources and 0 facts: every source was a `news.google.com/rss/articles/CBMi…` redirect shell answering HTTP 200 with a consent page. `ok` was describing transport, not usability. Now `ok` means usable — aggregator links rejected by pattern, thin bodies and zero-extraction pages classified distinctly, and topic selection skips aggregator-only topics with a stated reason instead of emitting a guaranteed-unpublishable draft.
- [x] **[S308][NEWS/P0][DEFECT FIXED] Resolutions were structurally discarded, so the accountability mechanic could never fire.** `buildLedgerFromDays()` passed only `predictions`, never `resolutions`, so grading was erased on every `--rebuild` — meaning the public track record, the standing directives, and the "publicly graded" line on every page could not work as built. Fixed with a committed `data/news-desk/resolutions.json`, `planLedgerEntries()` placing each grade in the first published day at or after it (plus a trailing entry so grading during a publishing gap cannot vanish), and validation at the rebuild boundary. A self-test now rebuilds a ledger with a resolution and asserts `personaTrackRecords().rex.correct === 1` — an assertion that fails against the old code.
- [x] **[S308][NEWS/P0] Add the three newsroom roles.** THE EDITOR (runs/spikes, assigns cast), STANDARDS (binds every claim to a cited source), CORRECTIONS (grades against evidence). Mechanized where checkable: `runStandards()` blocks a figure asserted in commentary that appears in no cited fact — a real hallucination proxy — plus uncited stances, sources outside the ingested set, and ungradeable predictions. `editorialReview()` is the refusal mechanism that makes safe autonomy reachable; `--promote` is gated on it.
...
- [x] **[S308][NEWS/P0][FOUNDER] Predictions were all long-horizon, so the desk could never be shown wrong.** All four published predictions land 326–510 days out: individually falsifiable, collectively unfalsifiable on any timescale a reader checks. Standards now blocks an all-long-horizon story (at least one call within `NEAR_TERM_DAYS` = 90), and the drafter proposes a staggered 45/120/240-day ladder instead of the flat 6-month default that would have perpetuated it. Published dates stand as written — re-dating a live prediction to flatter the record would be the worst possible "correction".
- [x] **[S308][NEWS/P0][FOUNDER] The desk was structurally incapable of being funny.** Every story required ≥2 facts, ≥2 stances, ≥1 dated prediction and a 40–110 word summary, so a sharp line about a viral moment could not publish without bolting a forecast onto the joke — and the radar actively filtered spectacle out as uncastable. Added six formats (Argument · Quick Take · Signature Bit · Roast · Plainly · Verdict) with per-format bars; predictions now belong to formats that make a claim about the future. Each persona owns a recurring column (REX *The Overhang*, MARA *The Receipt*, DOT *The Chart*, VERA *3 A.M.*, ECHO *Rhymes With*, JUNO *Name The Person*). Added a `spectacle` beat with ECHO/VERA able to carry it. **What never relaxes: a stated fact must be real and cited in every format — humour is licensed, invention is not.**
- [ ] **[S308→S309][NEWS/P1] Prove the light formats are actually good.** The machinery permits humour; nothing funny has shipped. The first roast is where we learn whether the voices are genuinely entertaining or merely structurally allowed to be. `buildLedgerFromDays()` rebuilds the prediction ledger from committed days and passes **only `predictions`**, never `resolutions`, so any grading would be erased on the next `--rebuild`. `personaTrackRecords()` and `personaForm()` both derive from resolutions, which means the public track record, the persona standing directives, and the "every prediction is publicly graded" claim printed on every page **cannot work as built**. Live state: ledger depth 1 · 4 open predictions · **0 resolutions** · every persona `unproven`. The earliest prediction is not due until 2027-06-30, so this would have stayed invisible for roughly a year. Fix: persist resolutions in a source that survives rebuild (a committed resolutions ledger keyed by prediction id), and add a grading path that writes them.
