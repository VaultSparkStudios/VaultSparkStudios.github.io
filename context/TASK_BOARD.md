...
Last updated: 2026-06-15 (Session 201 — fresh /audit → /implement 9/10 shipped + 1 premise-false WIN · build:check gates green)

## Now (S201 runway — pre-loaded)

- [ ] **[S201][VERIFY/P0] Confirm S201 wave on prod.** On a real browser (datacenter curl 403 = benign CF challenge): (a) `/journal/dispatches/` — sign in → classified section reveals 3 session-intelligence entries with rank note; (b) `/ranks/` or `/vault-member/` (signed-in) → "Share Rank" button appears bottom-right, tapping opens Web Share or copies to clipboard; (c) `/ignis/` — ask 2+ questions → "Synthesize my session →" button appears, clicking opens SESSION DIGEST card with topic list + deduped source chips; (d) homepage → vault-climbers strip is hidden (expected until RLS relaxed); (e) `/pathways/builders/` and others render correctly from data-driven source. Never assume push==deploy ([[feedback_skip_ci_tip_strands_cf_pages_deploy]]).
- [ ] **[S201→][INFRA/P2] VAULT CLIMBERS RLS — wire member read without exposing full table.** `build-rank-climbers.mjs` gets 400 from anon key (RLS). Options: (a) create `public_rank_climbers` Postgres VIEW with only `username, rank_name, points` + grant anon read; or (b) Supabase edge fn `rank-climbers` running as service role + returning curated list. Homepage strip auto-populates once wired. ~1h.
