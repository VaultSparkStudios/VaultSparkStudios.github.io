...
Last updated: 2026-05-28 (Session 171 — long-tail visual proof + RUM export diagnostics + runway truth cleanup; build:check green)

## Done (Session 170 — long-tail posture + disclosure gates)

- [x] **[S170][CONTENT/P1] LONGTAIL-PAGE-COPY-IMMERSION-PASS — DONE.** Added `scripts/check-longtail-studio-posture.mjs`, wired it into `build:check`, and updated representative project, legal/support, FAQ, and journal copy to carry the professional creative studio posture. **DONE S170**
- [x] **[S170][PROCESS/P1] INLINE-STYLE-EXTRACTION-GENERATOR-HARDENING — DONE.** `scripts/extract-inline-styles.mjs` now supports `--check`, `--list-targets`, and explicit `--targets=` validation; documented the supported workflow in `docs/STUDIO_THEME_EVOLUTION_SYSTEM.md`. **DONE S170**
- [x] **[S170][SECURITY/P1] AI-DISCLOSURE-LOCAL-FIRST-ALIGNMENT — DONE.** `/privacy/` and `/terms/` now distinguish local cited Ask IGNIS retrieval from model-backed gated features; `scripts/check-ai-disclosure-alignment.mjs` prevents the old blanket Anthropic claim from returning. **DONE S170**
- [x] **[S170][UX/P2] THEME-PRIMITIVE-ADOPTION-PASS — DONE.** Applied `.vs-immersive-band`, `.vs-section-kicker`, `.vs-signal-grid`, and `.vs-proof-note` to representative long-tail pages, and extended `check-studio-theme-evolution.mjs` to verify HTML usage. **DONE S170**

## Done (Session 171 — long-tail visual proof + RUM export diagnostics + runway truth)

- [x] **[S171][PROOF/P1] LONGTAIL-SCREENSHOT-PROOF — DONE.** `scripts/capture-longtail-visual-proof.mjs` (Playwright + sharp, local static server) captures desktop/mobile screenshots for `projects/vorn/`, `/privacy/`, and `journal/community-enters-the-vault/` into `docs/visual-proof/longtail-s171/` with a `manifest.json` carrying status/bytes/non-blank-score/page-errors per shot. `scripts/check-longtail-visual-proof.mjs` (self-test + manifest verifier) is wired into `build:check`; 6/6 screenshots green. **DONE S171**
- [x] **[S171][DATA/P1] RUM-EXPORT-PATH-DIAGNOSTICS — DONE.** `scripts/check-rum-export-path.mjs` inspects summary + local export source availability, writes `.cache/rum-export-diagnostics.json` with an explicit `nextAction`, and runs non-blocking in `build:check` so the empty RUM loop explains itself instead of silently falling back to synthetic perf. Self-test green; current status `empty · samples=0` with a documented unlock path. **DONE S171**
- [x] **[S171][PROCESS/P1] RUNWAY-TRUTH-CLEANUP — DONE.** Closed the stale S168 LEGACY-INTELLIGENCE carry with S169 evidence and regenerated the genius list so the next list promotes current work instead of already-shipped carries. **DONE S171**

## Now (Session 172 runway)

- [ ] **[S172][DATA/P1] RUM-SAMPLE-UNLOCK.** Diagnostics now name the exact gap (no local export source). Get at least one recent production Worker RUM window exported into `data/rum-raw.ndjson` (or set `RUM_RAW_PATH`), run `npm run rum:summary`, and let `check-perf-budget --source=rum` go authoritative. *(Founder action: production RUM export access.)*
- [ ] **[S172][MEMBERSHIP/P1] MEMBERSHIP-ASSET-ORPHAN-DECISION.** Rewire or retire the 3 feature-bearing membership/vaultsparked orphan assets after founder confirmation, preserving SOUL #2.
- [ ] **[S172][UX/P2] LONGTAIL-VISUAL-PROOF-REVIEW.** Founder review of the new `docs/visual-proof/longtail-s171/` desktop/mobile screenshots; promote the primitive rhythm to more long-tail surfaces if it reads well.
