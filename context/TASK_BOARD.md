...
- [x] **[S308][TRUTH/P0] Harden authorship disclosure so no reader can mistake The Desk for human writing.** Founder directive. Five gaps found (card said "AI SIGNAL" = topic not author; feed author read as a normal masthead; items carried no author; "experimental" absent; bylines unlabelled) — and `check-ai-disclosure-alignment` never looked at /news. Now enforced at five layers: pre-content banner, "AI persona" at every attribution, JSON-LD `Organization` author + `creditText`, feed author + per-item authors + `_vaultspark_disclosure`, and "WRITTEN BY AI" on the card. Full reasoning: D-S308.13.
- [x] **[S308][TRUTH/P0] Gate the disclosure so it cannot regress.** `check-news-ai-disclosure.mjs` (17 self-tests, in build:check), mutation-tested red on the exact "AI SIGNAL" wording. Its own v1 passed vacuously by reading the renderer's source (where the byline is a template literal) — it now renders a real card. D-S308.14.
- [x] **[S308][NEWS/P1] Bridge the radar to an authored edition.** `news-draft-edition.mjs` (self-tests) fills every deterministic field — cast, standing, sourced facts, edition, prediction ids/dates — leaving only judgment fields blank. `--promote` fails closed. Zero model calls: CANON-015 keeps authoring on the Max Plan. D-S308.15.
- [x] **[S308][NEWS/P0] Fix a self-inflicted honesty bug in the drafter.** First live run reported 4 "ok" sources and 0 facts — every source was a `news.google.com` redirect shell answering 200. `ok` described transport, not usability. Now aggregator links are rejected and aggregator-only topics skipped with a stated reason. D-S308.16.
...
- [x] **[S308][NEWS/P0][FOUNDER] The desk was structurally incapable of being funny.** Every story required ≥2 facts, ≥2 stances and ≥1 dated prediction, so a sharp line about a viral moment could not publish without bolting a forecast onto the joke — and the radar filtered spectacle out as uncastable. Six formats now carry per-format bars, each persona owns a recurring column, and a `spectacle` beat is carried by ECHO/VERA. Facts stay real and cited in every format. Full reasoning: D-S308.20.
