...
Last updated: 2026-08-14 (Session 316 fixed the coupled deploy-currency truth chain — blind git-depth gate, shallow-clone false `diverged`, and a producer/reader field mismatch — un-inverted an E2E gate that failed only when production was healthy, added push retry to both publishing crons, and restored three local-ahead surfaces regressed by inbound propagation; build:check 295/295, Playwright compliance 18/18, doctor blockingFailing 0)
