...
Last updated: 2026-06-12 (Session 191 — /goal chain: /start → /audit → /implement → /closeout · 4 shipped / 1 deferred-with-evidence)

## Now (S191 runway — pre-loaded)

- [ ] **[S191][VERIFY/P1] Confirm S191 features on prod after deploy.** (a) `/api/citation.json` is 200 on pages.dev with 4 sourced claims + suggestedCitation; (b) `agents.json` discovery block includes `citation`; (c) `.well-known/llms.txt` has the Citation line; (d) `/api/public-status.json` shows asOf today (not 2026-05-22) + real repo counts; (e) `/status/` proof tile surfaces the funnel-summary feed. Verify via pages.dev origin; never assume push==deploy.
- [ ] **[S191][P1] SEED-ROT FOLLOW-UP — staging-health + security-posture.** The new seed-rot WARN flagged `api/staging-health.json` at 92% of its 168h window and `api/security-posture.json` at 54% of its 720h window. `staging-health` is the urgent one (≈14h from stale). Give each a real generator (like `build-public-status.mjs`) or confirm its refresh workflow runs, so neither silently drags trustScore.
- [ ] **[S191→][FEATURE/P2·SEC] Oracle per-cluster feedback (deferred-with-evidence).** Blocker: Worker `RUM_UX_EVENTS` is an EXACT-match Set, so dynamic per-cluster keys silently drop at the edge (S186 class). Add a BOUNDED prefix-rule for the `oracle-answer:` family (charset + length capped) to `cloudflare/security-headers-worker.js` + a `worker.unit.spec.js` case FIRST, then wire the frontend emit (`oracle-answer:helpful:<clusterId>`) + `deriveSummary` prefix-sum. `rollup-rum-ux updateOracleFeedback` already parses `clusterKey`. Low ROI until the funnel has volume — do it when traffic warrants.
- [ ] **[S189][CONTENT/P1·FOUNDER] Review + publish the forge devlog draft.** Re-run `node scripts/draft-weekly-forge.mjs` for SOUL-voice output, then founder reviews + publishes to `journal/` to clear the stale journal warn-gate.
...
- [ ] **[SIL][P2] POSTURE-FEED GENERATOR SWEEP.** Give `staging-health` + `security-posture` real generators (like `build-public-status.mjs`) so NO status-proof feed is a hand-seed that can silently rot. First step: `scripts/build-staging-health.mjs` deriving from the existing staging probe.
- [ ] **[SIL][P2] BOUNDED WORKER PREFIX-RULE PRIMITIVE.** Add a reusable `prefixAllowlist(family,{charset,maxLen})` helper to `cloudflare/security-headers-worker.js` so dynamic RUM families (per-cluster Oracle feedback, future ones) can ship safely without loosening the global exact-match Set. Unblocks oracle-per-cluster-feedback.
