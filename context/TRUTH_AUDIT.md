...
| Dimension | Score | Evidence |
|---|---:|---|
| Schema alignment | 5 | One additive field: `api/commit-map.json` entries gain `visitorFacing` (boolean). Every existing consumer keeps parsing. The narrative feed's shape is unchanged — only its membership shrank. `news-publish.yml` steps gain an `ok` output alongside the existing `status`, which is preserved for the slot summary. |
| Prompt/template alignment | 5 | No prompt or template surface changed. |
| Public claim accuracy | 4 | **A live public surface was publishing internal subject matter and now is not.** The homepage returning-visitor strip carried *"Refined resync after publisher race (attempt 1)."* — one of 13 chore-derived sentences out of 24 on the feed. Deduction rather than a 5: the surface was wrong for months, the filter written to prevent exactly this was dead code the whole time, and no gate noticed — it was found by reading the producer against the reader. |
| Internal consistency | 4 | Producer and reader now share one contract (`visitorFacing`) shipped in a single change, with an absent field treated as not-publishable rather than defaulting open — the failure mode where a reader keys on a field its producer never writes. Unchanged and still boarded: the three rank ladders and three intent taxonomies from S343. |
| Evidence integrity | 5 | The session's central claim was verified against a live run log (`34063581495`), not inferred from reading YAML — the log shows which steps actually executed. The new gate was replayed against the **pre-fix** file as a negative control and caught all 7 historical guards, so its green on the fixed tree means something. An audit candidate was **disproved** by reading the implementation before it could ship. |
| Credential status honesty | 3 | **Unchanged from S343 and still the worst reading here.** `check-secrets` reports `supabase.admin ✓ READY 2/2` for a key scoped to `ckwtolofoqzrqouqkmvs` while this site ships `fjnpzjjyhnpmunfoycrp`. This session did not fix it — it routed around it: `deploy-member-newsletter.mjs` pins the project ref rather than deriving it from that slot, precisely because the obvious derivation would have deployed this site's member newsletter into a sibling tenant. The gateway fix remains studio-ops' via the S344 Ark cargo. |
| Audience voice integrity | 4 | The S343 finding is closed structurally rather than by a word blocklist: a commit that touched no visitor-facing path earns no public sentence whatever its type. Deduction, honestly: surviving sentences are still authored from commit subjects, so phrasing like *"repair the intelligence-budget artifact resync-derived does not cover"* remains engineer-voice on a public surface. The leak class is closed; the register is not. |
| Completion-claim discipline | 5 | The newsletter is recorded everywhere as **not sending** — the tooling shipped, the execution blocked on a permission, and the cron will fail again on 2026-10-02. The Desk is recorded as **still stale**, an escalation rather than a fix. Both headline fixes are stated as verified by self-test, negative control and local regeneration and **not** by a live scheduled run, because none has fired yet. `--verify` deliberately stops at the 404→401 boundary rather than proving a 200 by mailing every member. |

**Net:** the standing truth risk is unchanged — the credential audit still reports READY for a key that cannot authenticate, and this session worked around it rather than fixing it, because the fix is a sibling repo's. What the session removed is subtler: a guard that reported a step's success when that step had never run. Nothing false was ever published through it, but that was luck rather than design, and the two things that stopped it were both bugs a maintainer would have removed.

## S343 A Claimed Bot Filter, a Sold Cancel Path, and a READY Credential That 401s (2026-09-04)

| Dimension | Score | Evidence |
|---|---:|---|
| Schema alignment | 5 | No public schema broke. The funnel beacon row gains a `bot` boolean; every existing consumer keeps parsing, and the field is additive. `/login`'s JSON failure contract is unchanged byte-for-byte for non-browser clients — only the `Accept: text/html` branch is new. |
| Prompt/template alignment | 5 | No prompt or template surface changed. |
| Public claim accuracy | 4 | **Two standing public claims were false and are now true.** `/stats/ecosystem/` has advertised "bots separated from people" for months while the funnel applied no such filter — the claim now matches the code. The membership copy promises "Cancel anytime" while `customer-portal-session` had **zero callers** and the button was `typeof`-guarded into silence; the path now exists. Deduction, not a 5: both claims were live and wrong for an unknown period, and neither was caught by a gate — they were found by reading the code against the copy. |
| Internal consistency | 4 | `vs_visit_count` had two writers with incompatible meanings (per-path in `intent-state.js`, per-session in `returning-visitor-digest.js`) while every trust and cadence threshold on the site reads it; the session-scoped writer now owns it outright. Still inconsistent and boarded: **three rank ladders** (`membership-entitlements.json` / `rank-economy.json` / `build-rank-climbers.mjs`) disagree, so a member at 60 points has a different rank depending on which surface they read; and **three intent taxonomies** disagree across `intent-state.js`, `membership-journey.js` and `pathways-router.js`. |
| Evidence integrity | 4 | Production was verified by fetching the **served** `portal-auth.js` and confirming the only surviving `kitSubscribe` is the comment explaining its removal — not by trusting the deploy's exit code. Both hash-bound receipts were re-checked after each of two rebases so the CANON-053 evidence binds to the tree that shipped. Deduction: the visual review covers **2 of 84** captures, and the `vault-member` portal — this session's largest change — is absent from the theme matrix entirely; the receipt states both limits in those words rather than implying full coverage. |
| Credential status honesty | 3 | **The worst reading on this table, and it is a live defect.** `check-secrets --audit` reports `supabase.admin ✓ READY 2/2` for a key that returns **401 Invalid API key** against this site. It is valid and unexpired — and scoped to a different project (`ref: ckwtolofoqzrqouqkmvs` vs the shipped `fjnpzjjyhnpmunfoycrp`). Presence is being reported as validity, on the surface an agent consults before declaring work blocked. It also means the Obelisk ceremony would fail at its truth reads *after* a human completes the passkey flow. Boarded `[S343][SEC/P0]`; the fix belongs in studio-ops and needs Ark cargo. (D-S343.4) |
| Audience voice integrity | 3 | The homepage IGNIS chip published *"The studio keeps resync after publisher race"* — a chore commit about a rebase collision — as the first sentence under the studio name. A surface fed by commit history is a publishing surface and has no editor in its path; the `publicNote`/`publicNextStep` overrides that fixed the sibling surfaces are not consulted here. Found in the pixel review, deliberately not fixed under a frozen tree. (D-S343.5) |
| Completion-claim discipline | 4 | The registration fix is described everywhere as verified by the gate, three suites and the served asset — and **not** as user-proven, because the plan's own Phase 0 gate is a human signup in a clean profile and nobody has run it. Phases 3–7 are stated as untouched in the commit message, the handoff and the SIL rather than allowed to read as finished. Deduction: I twice captured receipts against a tree that had not converged, which would have bound evidence to something other than what shipped had `check-receipt-ordering` not caught it. |

**Net:** the session removed two false public claims and added one honest blocker to the record. The standing truth risk is no longer the site's copy — it is the **credential audit**, which reports READY for a key that cannot authenticate, on exactly the surface an agent trusts when deciding whether it is blocked.
...
### S333 review-cycle addendum — what the session got wrong about itself

- **A settings change silently reverted and was only caught by re-checking.** The Opus 5 model pin written at session start read back as an alias (`opus[1m]`) at the end. Re-pinned explicitly. One of three things the founder explicitly asked for had regressed without any error.
- **A claim was made twice in opposite directions before being measured.** The receipt/manifest divergence was first recorded as an unmitigated design flaw, then corrected to a pure S333 regression, and finally settled by counting 24 commit tips: 19 coherent, 5 drifted, all five drifted tips cron commits pre-dating the session. Both earlier entries are kept and superseded rather than edited away.
