...
// homepage for visitors with vs_visit_count ≥ 2. Reads api/recent-ships.json
// (founder-approved changelog titles, S358), filters to entries newer than
...
(function () {
  ...
  // S358: the READER changelog projection, not the commit-derived narrative —
  // the narrative's sentences are git subjects ("Refined record the three board
  // items worked."), and returning visitors were shown them as "New:".
  var FEED_URL = '/api/recent-ships.json';
  var MAX_SHOWN = 2;
  ...
  function render(entries) {
    ...
    entries.slice(0, MAX_SHOWN).forEach(function (e) {
      ...
      var text = document.createTextNode(esc(e.title || ''));
  ...
  function flattenEntries(data) {
    var all = Array.isArray(data && data.ships) ? data.ships : [];
    // Filter to entries newer than last visit; sort newest-first.
    return all.filter(function (e) {
      var t = e && e.date && e.title ? new Date(e.date).getTime() : 0;
      return t > prevTs;
    }).sort(function (a, b) {
