...
(function (window) {
  ...
  // The vendor SDK is absent on pages that load this client without the
  // supabase-js bundle. Rejecting an un-awaited promise here raised an
  // unhandled rejection on every such load (a real pageerror, not a warning).
  // Resolve a degraded authority instead: consumers await the same contract,
  // see `degraded: true` plus the reason, and can surface an honest message
  // rather than silently rendering a signed-out screen.
  if (!window.supabase) {
    var unavailable = new Error('Supabase data client did not load.');
    window.VSSupabaseUnavailable = unavailable;
    window.VSObeliskIdentity = null;
    window.VSCompatibilitySession = null;
    window.VSAuthReady = Promise.resolve({
      authenticated: false, identity: null, session: null, degraded: true, error: unavailable,
    });
    // Announce on a later tick so deferred listeners registered after this
    // script still receive the event.
    setTimeout(function () {
      window.dispatchEvent(new CustomEvent('vs-auth-ready', {
        detail: { authenticated: false, degraded: true, error: unavailable.message },
      }));
    }, 0);
    return;
  }
