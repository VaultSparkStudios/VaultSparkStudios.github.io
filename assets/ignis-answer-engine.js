...
(function () {
  ...
  function ensureStyles() {
    // S275 CLS root-fix: the engine stylesheet now ships statically in
    // assets/style.css (see "Ask IGNIS answer engine" block) so the section is
    // styled at first paint instead of compressing when this module loads.
    // Fallback probe: if a stripped/legacy page lacks the static block, inject
    // the minimal container rule so the surface stays usable (never pretty-
    // broken), and report nothing — layout correctness beats completeness here.
    if (document.getElementById('vs-ignis-answer-style')) return;
    try {
      var probe = document.createElement('div');
      probe.className = 'vs-ask-ignis';
      probe.hidden = true; // UA display:none — no runtime inline style (contract-safe)
      document.body.appendChild(probe);
      var styled = getComputedStyle(probe).borderRadius === '16px';
      document.body.removeChild(probe);
      if (styled) return;
    } catch (_e) {}
    var s = document.createElement('style');
    s.id = 'vs-ignis-answer-style';
    s.textContent = '.vs-ask-ignis{margin:2rem 0;padding:1rem;border:1px solid var(--line);border-radius:16px;background:rgba(255,255,255,.035)}';
  ...
  function mount(root, contextQueries) { // S210 #1: contextQueries = page-aware chips
    ...
    // S275 CLS root-fix: a static mount (oracle/index.html #ask-ignis) reserves
    // the panel's height at first paint; release the reservation now that the
    // real panel is in place (class-based per the runtime style contract).
    root.classList.add('vs-ignis-mounted');
