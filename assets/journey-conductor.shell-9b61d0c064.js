...
(function () {
  ...
  function style() {
    ...
    sheet.textContent =
      ...
      // S357 (audit P1-6): --gold is a TEXT-on-cream token in light mode
      // (#8a6000); using it as a button background measured 3.35:1. The button
      // surface pair (--btn-surface/--btn-ink, assets/style.css) is AA in every
      // theme and keeps the brand gold.
      '.vs-vault-bridge__primary,.vs-journey__action{background:var(--btn-surface,var(--gold,#ffc400));color:var(--btn-ink,#171103)}.vs-vault-bridge__secondary{border:1px solid color-mix(in srgb,var(--text,#fff) 25%,transparent);color:var(--text,#fff)}' +
      // max-height keeps the panel from swallowing the viewport; the overlay
      // registry reserves whatever height it ends up with so the footer legend
      // is never underneath it (audit P1-14).
      '.vs-journey{position:fixed;right:clamp(.75rem,3vw,1.5rem);bottom:clamp(.75rem,3vw,1.5rem);z-index:10020;width:min(390px,calc(100vw - 1.5rem));max-height:min(52dvh,560px);overflow:auto;overscroll-behavior:contain;padding:1.1rem;border:1px solid color-mix(in srgb,var(--gold,#ffc400) 42%,transparent);border-radius:22px;background:var(--panel-strong,#0f1120);color:var(--text,#f8fafc);box-shadow:0 24px 80px rgba(0,0,0,.32);backdrop-filter:blur(18px)}' +
