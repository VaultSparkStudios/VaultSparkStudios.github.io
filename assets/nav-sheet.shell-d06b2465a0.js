...
(function () {
  ...
  function init(openImmediately) {
    ...
    // CANON-041: body scroll lock — same iOS-safe pattern as nav-toggle.js.
    // `overflow:hidden` on body swallows fixed-overlay taps in iOS Safari;
    // `position:fixed` + restored offset is the reliable alternative.
    var savedScrollY = 0;
    function lockScroll() {
