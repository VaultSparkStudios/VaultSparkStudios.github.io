...
/* Sources: 25 files, parsed in declared order. */
  ...
  async function query() {
    try {
      if (window.VSIdentity && typeof window.VSIdentity.getSession === 'function') {
        ...
        if (sb && sb.user) {
          resolve({ userId: sb.user.id, email: sb.user.email, raw: sb });
        } else {
          resolve(null);
        }
      } else {
...
(function () {
  ...
  document.addEventListener('vs:session-ready', function (e) {
    ...
    if (detail.signedIn && detail.session && detail.session.raw) {
      load(detail.session.raw);
    }
  });
