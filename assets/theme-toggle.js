...
(function () {
  ...
  function activateAsyncCss() {
    document.querySelectorAll('link[data-vs-async-css][media="print"]').forEach(function (link) {
      link.setAttribute('media', 'all');
      link.removeAttribute('data-vs-async-css');
    });
  }
  ...
  function applyTheme(theme) {
    ...
    var themeClass = THEME_CLASSES[resolvedTheme];
    ...
    if (
      ...
      (!themeClass || (root.classList.contains(themeClass) && body.classList.contains(themeClass)))
    ) {
  ...
  function injectThemePicker() {
    ...
    arrow.textContent = '▼'; // ▼ — textContent, not innerHTML (TT sink burndown S176)
