  ...
  function init() {
    ...
    if (!document.getElementById(styleId)) {
      ...
      st.textContent =
        ...
        '.vs-nav-sheet{position:fixed;left:0;right:0;bottom:0;max-height:60dvh;background:var(--mobile-nav-bg,rgba(5,6,14,0.99));border-top:1px solid var(--mobile-nav-border,rgba(255,255,255,0.06));border-radius:18px 18px 0 0;z-index:2147483641;transform:translateY(100%);transition:transform 280ms cubic-bezier(.4,0,.2,1);overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;padding:0.6rem 1.1rem calc(1.4rem + env(safe-area-inset-bottom,0px));color:var(--text);display:flex;flex-direction:column;box-shadow:0 -18px 60px rgba(0,0,0,0.6)}' +
    ...
    /* iOS-safe scroll lock matching nav-toggle.js: `overflow:hidden` on body
     * swallows fixed-overlay taps in iOS Safari; pin body with position:fixed
     * instead, restoring scroll offset on close. */
    var savedScrollY = 0;
    function lockScroll() {
